...
const Be::ClassInfo* EveBoosterSet2::ExposeToBlue()
{
    EXPOSURE_BEGIN( EveBoosterSet2, "" )
		...
		// Be::READ (not PERSISTONLY) is deliberate: platformtools' make-compatible-with-trinity-booster-update
		// branch detects this build via hasattr(trinity.EveBoosterSet2, "boosters"), which requires the
		// attribute to not be Be::HIDDEN. WRITE is still withheld since nothing should reassign this from Python.
		MAP_ATTRIBUTE( "boosters", m_boosters, "Per-booster source data: functionality, atlas indices, trail flag, light scale", Be::READ | Be::PERSIST )
