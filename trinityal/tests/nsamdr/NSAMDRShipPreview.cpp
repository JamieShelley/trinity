...
Texture2D gEnvironmentMap : register(t3);
...
float3 EvaluateLight(float3 n, float3 v, float3 l, float3 albedo, float3 lightColour, float intensity, float roughness, float specularScale)
{
    float nDotL = saturate(dot(n, l));
    if (nDotL <= 0.0) return float3(0.0, 0.0, 0.0);
    float3 h = normalize(v + l);
    float3 f0 = lerp(float3(0.035, 0.04, 0.045), saturate(albedo * 0.28 + 0.04), saturate(specularScale));
    float3 f = FresnelSchlick(saturate(dot(h, v)), f0);
    float d = DistributionGGX(n, h, roughness);
    float g = GeometrySmith(n, v, l, roughness);
    float3 specular = d * g * f / max(4.0 * saturate(dot(n, v)) * nDotL, 1.0e-4);
    float3 diffuse = albedo * (1.0 - f) / 3.14159265;
    return (diffuse + specular * specularScale) * lightColour * intensity * nDotL;
}
...
float3 SampleEnvironment(float3 direction)
{
    ...
    if (gEnvironment.x > 0.5)
    {
        ...
        return max(gEnvironmentMap.SampleLevel(gTextureSampler, uv, 0.0).rgb, 0.0);
    }
...
float4 PSMain(VSOutput input) : SV_TARGET
{
    ...
    float3 neutralPaint = float3(0.045, 0.105, 0.145);
    float3 sampledAlbedo = gAlbedo.Sample(gTextureSampler, uv).rgb;
    float3 albedo = gMaterial.x > 0.5 ? sampledAlbedo : neutralPaint;
    ...
    float damage = saturate(max(input.stretchHint, derivativeDamage * 0.40));

    int mode = (int)round(gControls.x);
    if (mode == 2) return float4(DamageHeatmap(damage), 1.0);

    float3 mappedNormal = geometricNormal;
    if (gOptions.x > 0.5)
    {
        float3 normalSample = gNormalMap.Sample(gTextureSampler, uv).xyz * 2.0 - 1.0;
    ...
    float sourceMicro = Fbm(uv * 123.0 + sampledAlbedo.rg * 7.0);
    ...
    float reconstructed = (mode == 1 || mode == 4) ? saturate(stochastic + neural * 0.24) : stochastic;
    float reconstructionAmount = (mode == 1 || mode == 3 || mode == 4) ? saturate(damage * gControls.y) : 0.0;

    float micro = lerp(sourceMicro, reconstructed, reconstructionAmount);
    ...
    float3 n = normalize(mappedNormal - microGradient * gSurface.x * reconstructionAmount);
    float3 v = normalize(gCameraTime.xyz - input.worldPos);

    float3 pgs = gOptions.y > 0.5 ? gPgsMap.Sample(gTextureSampler, uv).rgb : float3(0.0, 0.45, 0.55);
    float roughness = saturate(0.62 - pgs.g * 0.42 + gSurface.w);
    roughness = clamp(roughness, 0.09, 0.92);
    float specularScale = saturate(gSurface.z * (0.35 + pgs.b * 1.15));

    float3 colour = 0.0;
    colour += EvaluateLight(n, v, normalize(-gKeyLight.xyz), albedo, float3(1.00, 0.88, 0.72), gKeyLight.w, roughness, specularScale);
    colour += EvaluateLight(n, v, normalize(-gFillLight.xyz), albedo, float3(0.32, 0.55, 1.00), gFillLight.w, roughness + 0.08, specularScale * 0.65);
    colour += EvaluateLight(n, v, normalize(-gRimLight.xyz), albedo, float3(0.18, 0.48, 1.00), gRimLight.w, roughness, specularScale);

    float hemisphere = saturate(n.y * 0.5 + 0.5);
    float3 ambientColour = lerp(float3(0.018, 0.024, 0.034), float3(0.085, 0.15, 0.25), hemisphere);
    float3 environmentDiffuse = SampleEnvironment(n);
    colour += albedo * (ambientColour + environmentDiffuse * (0.16 * gEnvironment.y)) * gMaterial.w;

    float fresnel = pow(1.0 - saturate(dot(n, v)), 4.0);
    float3 reflectedEnvironment = SampleEnvironment(reflect(-v, n));
    colour += reflectedEnvironment * gEnvironment.y * gEnvironment.w *
        (0.10 + fresnel * (0.34 + specularScale * 0.52));

    colour += (micro - 0.5) * 0.12 * reconstructionAmount;
    colour *= gMaterial.z;
    colour = colour / (1.0 + colour);
    colour = pow(saturate(colour), 1.0 / 2.2);
    return float4(colour, 1.0);
    ...
    float& hitDistance)
{
    ...
    XMFLOAT3 eye, target;
    BuildCameraMatrices(state, width, height, world, view, projection, eye, target);
    ...
    PreviewResources& resources)
{
    ...
    else
    {
        success = LoadWicTexture(
            device, albedoPath, resources.albedoView, resources.textureWidth, resources.textureHeight, "albedo image");
    ...
    if (!normalPath.empty())
    {
        resources.hasNormalMap = LoadWicTexture(
            device, normalPath, resources.normalView, resources.normalWidth, resources.normalHeight, "normal map");
        if (!resources.hasNormalMap) return false;
    }
    if (!pgsPath.empty())
    {
        resources.hasPgsMap = LoadWicTexture(
            device, pgsPath, resources.pgsView, resources.pgsWidth, resources.pgsHeight, "PGS material map");
        if (!resources.hasPgsMap) return false;
    }
    if (!environmentPath.empty())
    {
        resources.hasEnvironment = LoadWicTexture(
            device,
            environmentPath,
            resources.environmentView,
            resources.environmentWidth,
            resources.environmentHeight,
            "EVE environment image");
        if (!resources.hasEnvironment) return false;
    }
    return true;
}
...
void ProcessHotkeys(PreviewState& state, HWND hwnd, const ObjMesh& mesh)
{
    ...
    if (!io.WantCaptureKeyboard)
    {
        ...
        if (KeyPressed('3')) { state.mode = 3; state.previousEnabledMode = 3; }
        if (KeyPressed('4')) { state.mode = 4; state.previousEnabledMode = 4; }
        if (KeyPressed(VK_SPACE))
        {
            if (state.mode == 0) state.mode = state.previousEnabledMode;
    ...
    const std::string& environmentPath)
{
    ...
    ImGui::SetNextWindowSize(ImVec2(640.0f, 880.0f), ImGuiCond_FirstUseEver);
    ImGui::Begin("NSAMDR Real EVE Ship Controls");

    ImGui::TextUnformatted("UV-stretch detection and selective material microdetail reconstruction");
    ...
    if (resources.hasPgsMap) ImGui::TextWrapped("PGS: %s (%u x %u)", pgsPath.c_str(), resources.pgsWidth, resources.pgsHeight);
    if (resources.hasEnvironment)
    {
        ImGui::TextWrapped("EVE nebula environment: %s (%u x %u)", environmentPath.c_str(), resources.environmentWidth, resources.environmentHeight);
    }
    else
    {
    ...
    if (resources.hasPgsMap) ImGui::Checkbox("Use EVE PGS material map", &state.usePgsMap);

    ImGui::Separator();
    ImGui::Checkbox("Use albedo texture", &state.useTexture);
    ImGui::Checkbox("Flip texture V (V)", &state.flipV);
    ...
    ImGui::TextUnformatted("Mouse: RMB orbit | MMB pan | Shift+MMB fine pan | wheel zoom-to-cursor | Ctrl+wheel fine zoom");
    ImGui::TextUnformatted("Double-click a hull panel to make it the orbit and zoom focus point.");
    ImGui::TextUnformatted("Keys: 0-4 modes | Space compare | F/Home frame ship | V flip UV | R reset | F9 capture | Esc exit");
    ImGui::TextWrapped("Damage-mask yellow/red areas are detected from the OBJ triangle UV Jacobian plus screen-space derivatives.");
    ImGui::TextWrapped("Mode 0 preserves the mapped source result. Modes 1, 3 and 4 add replacement microdetail only where stretching is detected.");
    ...
    float elapsedSeconds)
{
    XMMATRIX world, view, projection;
    XMFLOAT3 eye, target;
    BuildCameraMatrices(state, resources.width, resources.height, world, view, projection, eye, target);
    ...
    constants.environment = XMFLOAT4(
        state.useEnvironment && resources.hasEnvironment ? 1.0f : 0.0f,
...
void RenderShip(ID3D11DeviceContext* context, const PreviewResources& resources, const PreviewState& state)
{
    ...
    ID3D11ShaderResourceView* environmentView = resources.environmentView.Get();
    ...
    context->PSSetShaderResources(3, 1, &environmentView);
    ...
    context->VSSetConstantBuffers(0, 1, &constantBuffer);
    context->PSSetConstantBuffers(0, 1, &constantBuffer);
    context->PSSetShaderResources(0, 4, textureViews);
    context->PSSetSamplers(0, 1, &textureSampler);
...
std::string BuildScreenshotPath(int mode)
{
    ...
    return "artifacts\\nsamdr\\real_obj_mode_" + std::to_string(mode) + "_" + std::to_string(milliseconds) + ".dds";
}
...
TEST_F(NSAMDRRendering, RealObjShipPreview)
{
    ...
    const std::string environmentPath = GetEnvironmentString("NSAMDR_ENVIRONMENT");
    ...
    PreviewResources resources;
    ASSERT_TRUE(CreatePreviewResources(device, swapChain, mesh, albedoPath, normalPath, pgsPath, environmentPath, resources));
    ...
    PreviewState state;
    state.useTexture = resources.hasExternalAlbedo;
    state.useNormalMap = resources.hasNormalMap;
    state.usePgsMap = resources.hasPgsMap;
    ...
    auto frame = [&]() {
        ...
        if (!ImGui::GetIO().WantCaptureMouse && ImGui::IsMouseDoubleClicked(0))
        {
        ...
        ProcessHotkeys(state, hwnd, mesh);
        if (state.autoOrbit)
        {
        ...
        DrawControlPanel(
            state, shipCatalog, hwnd, mesh, resources, albedoPath, normalPath, pgsPath, environmentPath);
