...
def load_checkpoint(
    path: Path,
    device: torch.device,
) -> tuple[NSAMDRV16, dict[str, Any]]:
    try:
        payload = torch.load(path, map_location=device, weights_only=False)
    except TypeError:
        payload = torch.load(path, map_location=device)
    if not isinstance(payload, dict) or payload.get("schema") != MODEL_SCHEMA:
        raise RuntimeError(f"V16 checkpoint schema mismatch: {path}")
    config_payload = dict(payload.get("config") or {})
    fields = V16Config.__dataclass_fields__
    config = V16Config(**{k: v for k, v in config_payload.items() if k in fields})
    config.validate()
    model = NSAMDRV16(config).to(device)
...
def promote_final(
    ...
    destination = final_dir / "nsamdr_v16.pt"
    ...
    if (
        ...
        raise RuntimeError(
            "V16 promoted checkpoint failed clean production architecture verification"
        )
