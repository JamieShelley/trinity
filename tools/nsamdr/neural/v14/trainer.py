...
class V14Trainer:
    def __init__(
        ...
        *,
        ...
        self.model = NSAMDRV14(config).to(device)

    def _autocast(self):
        if self.device.type != "cuda":
            return torch.autocast(device_type="cpu", enabled=False)
        dtype = torch.bfloat16 if self.amp_precision in {"auto", "bf16"} else torch.float16
        return torch.autocast(device_type="cuda", dtype=dtype)

    def _to_device(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        return {key: value.to(self.device, non_blocking=True) for key, value in batch.items() if isinstance(value, torch.Tensor)}

    def _loader(self, split: str, length: int, degradation: str, seed: int) -> DataLoader:
    ...
    def _validate(self, *, final: bool = False) -> tuple[list[dict[str, float]], dict[str, object]]:
        loader = self._loader("validation", self.config.validation_tiles, "clean", self.config.seed + 700)
    ...
    def _save_preview(self, epoch: int, phase: str) -> None:
        ...
        if self.live_preview_target_size > 0 and a.shape[0] != self.live_preview_target_size:
        ...
        folder = self.experiment_dir / "previews" / "live" / "candidates" / f"epoch_{epoch:04d}"
        folder.mkdir(parents=True, exist_ok=True)
        for name, image in (("A_authored.png", a), ("B_baseline.png", b), ("C_candidate.png", c), ("F_final.png", f)):
            cv2.imwrite(str(folder / name), image[:, :, ::-1])
        panels = []
        for label, image in (("A AUTHORED", a), ("B BASELINE", b), ("C V14 SR", c), ("F SELECTED", f)):
            panel = image.copy()
        ...
        pointer = {
            "schema": "NSAMDR_V14_LIVE_PREVIEW_V1",
            "epoch": epoch,
            "phase": phase,
        ...
        (live_root / "checkpoint_ready.json").write_text(json.dumps(pointer, indent=2) + "\n", encoding="utf-8")
        print(
            f"[live-preview] epoch {epoch} {phase}: A {target.shape[-1]} / B,C {outputs['candidate_albedo'].shape[-1]} "
            f"from LR {batch['lr_albedo'].shape[-1]} -> {folder / 'ABCF.png'}",
            flush=True,
        )
    ...
    def _train_candidate(self) -> tuple[Path, dict[str, object]]:
        ...
        for epoch in range(1, self.config.sr_epochs + 1):
            degradation = "clean" if epoch <= self.config.clean_epochs else "robust"
            loader = self._loader("train", self.config.tiles_per_epoch, degradation, self.config.seed + epoch * 17)
            self.model.train()
            ...
            checkpoint = self.experiment_dir / "checkpoints" / "candidate" / f"epoch_{epoch:04d}.pt"
            save_checkpoint(checkpoint, self.model, self.config, epoch=epoch, phase=f"sr-{degradation}", metrics=report)
            print(
                "  valid "
    ...
    def _train_selector(self, candidate_path: Path, candidate_report: dict[str, object]) -> tuple[Path, dict[str, object]]:
        ...
        for step in range(1, self.config.selector_epochs + 1):
            ...
            print(
                f"Epoch {epoch:03d}/{self.config.sr_epochs + self.config.selector_epochs:03d} "
                f"phase=selector lr={self.config.selector_learning_rate:.7f}", flush=True,
            )
            for index, batch in enumerate(loader, start=1):
                batch = self._to_device(batch)
                optimizer.zero_grad(set_to_none=True)
                ...
                if index == 1 or index % 24 == 0 or index == len(loader):
                    elapsed = max(time.perf_counter() - start, 1.0e-6)
                    print(f"  {index:4d}/{len(loader):4d} total={running/index:.6f} rate={index/elapsed:.2f}tile/s", flush=True)
            ...
            checkpoint = self.experiment_dir / "checkpoints" / "selector" / f"epoch_{epoch:04d}.pt"
            save_checkpoint(checkpoint, self.model, self.config, epoch=epoch, phase="selector", metrics=report)
            print(
                "  valid-final "
                f"global={float(report['medianGlobalRecovery'])*100:+.2f}% "
                f"edge={float(report['medianEdgeRecovery'])*100:+.2f}% "
    ...
    def run(self) -> dict[str, object]:
        ...
        print("=" * 72, flush=True)
        print("NSAMDR V14 HR-FIRST SR", flush=True)
        print(f"Model schema             : {MODEL_SCHEMA}", flush=True)
        print("Production authority     : B -> HR refinement C -> BenefitSelector F", flush=True)
        print(f"Training geometry        : {self.config.train_lr_size} LR -> {self.config.train_hr_size} HR", flush=True)
        print(f"Native validation samples: {len(self.native_samples)}", flush=True)
        print(f"Parameters               : {sum(p.numel() for p in self.model.parameters()):,}", flush=True)
        ...
        selector_path, final_report = self._train_selector(candidate_path, candidate_report)
        ...
        final_path = promote_final(selector_path, self.experiment_dir, final_report)
        return {
            "status": "completed",
            "qualified": True,
            "candidate": candidate_report,
