...
def tiled_inference(
    ...
    *,
    ...
    if lr_albedo.shape[0] != 1:
        raise ValueError("V14 tiled inference currently expects batch size 1")
    ...
    keys = (
        ...
        "candidate_albedo", "candidate_normal", "candidate_material",
        "albedo", "normal", "material", "selector_probability",
    )
    channels = {
        "baseline_albedo": 3, "baseline_normal": 2, "baseline_material": 3,
        "candidate_albedo": 3, "candidate_normal": 2, "candidate_material": 3,
        "albedo": 3, "normal": 2, "material": 3, "selector_probability": 1,
    }
    accum = {
        key: torch.zeros((1, channels[key], out_h, out_w), device=lr_albedo.device, dtype=torch.float32)
        for key in keys
    }
    weight = torch.zeros((1, 1, out_h, out_w), device=lr_albedo.device, dtype=torch.float32)
