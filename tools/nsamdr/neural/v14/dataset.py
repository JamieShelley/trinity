...
def native_family_samples(manifest: dict[str, Any], config: V14Config) -> list[dict[str, torch.Tensor | str]]:
    ...
    for family in list(manifest.get("families") or [])[: config.native_validation_max_families]:
        ...
        if not albedo_path.is_file() or not normal_path.is_file():
            continue
        albedo_u8 = _read_rgb(albedo_path)
        normal_u8 = _read_rgb(normal_path)
        if material_path.is_file():
            material_u8 = _read_rgb(material_path)
        else:
            material_u8 = np.zeros_like(albedo_u8, dtype=np.uint8)
        h, w = albedo_u8.shape[:2]
        side = min(h, w)
        ...
        y = (h - side) // 2
        x = (w - side) // 2
        albedo = albedo_u8[y:y+side, x:x+side].astype(np.float32) / 255.0
        normal = normal_u8[y:y+side, x:x+side, :2].astype(np.float32) / 127.5 - 1.0
        normal = _normalise_xy(normal)
        material = material_u8[y:y+side, x:x+side, :3].astype(np.float32) / 255.0
        ...
        result.append({
            "name": str(family.get("familyId") or albedo_path.stem),
