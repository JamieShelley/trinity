#!/usr/bin/env python3
"""V14.4 BenefitSelector retention diagnostic."""
...
def _latest_passing_multiregion(
    ...
    if not candidates:
        raise RuntimeError(
            "Selector Retention Mini requires a passing V14.4 Multi-Region SR Mini first"
        )
...
class SelectorRetentionDiagnostic:
    """Freeze a passing V14.4 candidate and train only BenefitSelector."""
    ...
    def run(self) -> tuple[int, Path]:
        ...
        if source_report.get("datasetFingerprint") != manifest.get("fingerprint"):
            raise RuntimeError(
                "Raven dataset changed since the passing V14.4 multi-region mini; rerun it first"
            )
        ...
        if len(train_records) < 2 or len(validation_records) < 2:
            raise RuntimeError(
                "Passing V14.4 multi-region mini no longer resolves its original Raven regions"
            )
        source_candidate = dict(source_report.get("candidate") or {})
        if source_candidate.get("passed") is not True:
            raise RuntimeError(
                "Passing V14.4 multi-region report has no qualified candidate block"
            )
        ...
        print("=" * 76, flush=True)
        print("V14.4 SELECTOR RETENTION MINI - DIAGNOSTIC ONLY", flush=True)
        ...
        if best_path is None or best_report is None:
            raise RuntimeError("V14.4 selector diagnostic produced no checkpoint")
...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="NSAMDR V14.4 selector retention diagnostic")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"[v14.4-selector] report      : {run_dir / 'report.json'}", flush=True)
    print(f"[v14.4-selector] diagnostics : {archive}", flush=True)
    print(f"[v14.4-selector] result      : {'PASS' if code == 0 else 'FAIL'}", flush=True)
    return code
