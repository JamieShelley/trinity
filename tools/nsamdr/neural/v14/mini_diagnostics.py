#!/usr/bin/env python3
"""Dispatch non-promotable V14.2 mini diagnostics.
...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="NSAMDR V14.2 focused Raven mini diagnostics")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"[v14.2-mini] report      : {run_dir / 'report.json'}", flush=True)
    print(f"[v14.2-mini] diagnostics : {archive}", flush=True)
    print(f"[v14.2-mini] result      : {'PASS' if code == 0 else 'FAIL'}", flush=True)
    return code
