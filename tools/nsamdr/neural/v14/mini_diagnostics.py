#!/usr/bin/env python3
"""Dispatch non-promotable V16.0 mini diagnostics.
...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="NSAMDR V16.0 focused Raven mini diagnostics")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"[v16.0-mini] report      : {run_dir / 'report.json'}", flush=True)
    print(f"[v16.0-mini] diagnostics : {archive}", flush=True)
    print(f"[v16.0-mini] result      : {'PASS' if code == 0 else 'FAIL'}", flush=True)
    return code
