...
def _prepare_dataset(args: argparse.Namespace, repo_root: Path) -> None:
    ...
    print("[v14.1-workflow] prepare authored Raven dataset: " + subprocess.list2cmdline(command), flush=True)
...
def _diagnostics(experiment: Path, root: Path) -> Path:
    ...
    base = diagnostics / f"{experiment.name}_V14_1_DIAGNOSTICS"
...
def _start_live_view(repo_root: Path, experiment: Path) -> subprocess.Popen[bytes]:
    ...
    print("[v14.1-workflow] live A/B/C/F viewer: " + subprocess.list2cmdline(command), flush=True)
...
def _write_architecture_participation(experiment: Path, trainer: V14Trainer) -> None:
    ...
    passed = bool(
        ...
        and contract.get("revision") == "V14.1"
        and active == ("baseline", "context_encoder", "context_adapter", "hr_refiner", "selector")
        and retired == ()
        ...
        and contract.get("contextUpsampling") == "bilinear-phase-neutral + HR 3x3 adapter"
        and contract.get("selectorUsesPhysicalMaps") is True
    )
    payload = {
        "schema": "NSAMDR_V14_ARCHITECTURE_PARTICIPATION_V2",
        "pass": passed,
    ...
    if not passed:
        raise RuntimeError("V14.1 architecture participation contract failed")


def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="NSAMDR V14.1 phase-neutral HR-first Raven workflow")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.training_mode != "quick":
        print("ERROR: V14.1 Full Training remains disabled until Raven HR-first candidate qualifies.", file=sys.stderr)
        return 2
    ...
    if experiment.exists() and any(experiment.iterdir()):
        raise RuntimeError(f"V14.1 experiments are immutable; choose new instead of reusing {experiment_id}")
    ...
    manifest = {
        ...
        "source": "V14.1 phase-neutral HR-first clean architecture",
    }
    ...
    if args.preview_device == "cuda" and device.type != "cuda":
        raise RuntimeError("V14.1 Raven Quick requested CUDA but CUDA is unavailable")
    ...
    try:
        ...
        print(f"[v14.1-workflow] FAILED diagnostics: {archive}", flush=True)
        raise
    finally:
        _stop_live_view(experiment, viewer)

    archive = _diagnostics(experiment, repo_root)
    print(f"[v14.1-workflow] diagnostics: {archive}", flush=True)
    if result["qualified"]:
        print(f"[v14.1-workflow] QUALIFIED {experiment_id}", flush=True)
        return 0
    print(f"[v14.1-workflow] REJECTED {experiment_id}: {result.get('failedStage')}", flush=True)
    return 2
