#!/usr/bin/env python3
"""Discover broader native authored Caldari battleship texture families for V16.

This is a CPU-only data-authority scan. It reads the EVE resource indexes and DDS
headers directly. It does not prepare render assets, convert GR2, convert DDS files,
enumerate environments, modify the canonical dataset, or train a model.
"""
...
REPORT_SCHEMA = "NSAMDR_V16_CALDARI_BATTLESHIP_DIVERSITY_SCAN_V1_INDEX_ONLY"
MINIMUM_RECOMMENDED_FAMILIES = 4
...
def _candidate_entries(
    ...
    anchor_key = str(anchor.canonical_key).casefold()
    anchor_asset = _asset_path(anchor.preferred_asset)
    anchor_parent = anchor_asset.rsplit("/", 1)[0] if "/" in anchor_asset else ""

    selected: list[eve.ShipCatalogEntry] = []
    seen: set[str] = set()
    for entry in entries:
        asset = _asset_path(entry.preferred_asset)
        path_match = "/ship/caldari/battleship/" in asset
        metadata_match = (
            "battleship" in str(entry.group_name or "").casefold()
            and "caldari" in str(entry.faction_name or "").casefold()
        )
        if not (path_match or metadata_match):
            continue
    ...
    selected.sort(
        key=lambda entry: (
            0 if str(entry.canonical_key).casefold() == anchor_key else 1,
            0 if "raven" in entry.display_name.casefold() else 1,
            0
            if anchor_parent
            and _asset_path(entry.preferred_asset).rsplit("/", 1)[0] == anchor_parent
            else 1,
...
def scan(args: argparse.Namespace) -> tuple[int, Path]:
    ...
    run_dir = make_run_directory(repo_root, "battleship_diversity")
    ...
    print(f"[v16-battleship-diversity] Run: {run_dir}", flush=True)
    print(f"[v16-battleship-diversity] EVE SharedCache: {cache_root}", flush=True)
    print("[v16-battleship-diversity] Reading resource indexes once...", flush=True)
    rows = builder._authoritative_rows(indexes)
    print(f"[v16-battleship-diversity] Indexed resources: {len(rows)}", flush=True)
    ...
    print(
        f"[v16-battleship-diversity] Candidate Caldari battleship entries: "
        f"{len(candidates)} (limit={args.limit})",
        flush=True,
    )
    print(
        "[v16-battleship-diversity] Fast path: index + DDS headers only; "
    ...
    for index, entry in enumerate(candidates, start=1):
        prefix = f"[v16-battleship-diversity] {index}/{len(candidates)}"
        print(f"{prefix} {entry.display_name} ({entry.canonical_key})", flush=True)
        record: dict[str, Any] = {
        ...
        try:
            ...
            sources: dict[str, Any] = {"albedo": albedo, "normal": normal}
    ...
    passed = len(unique_families) >= MINIMUM_RECOMMENDED_FAMILIES
    report = {
        "schema": REPORT_SCHEMA,
        "revision": "V16.0",
        "mode": "battleship-diversity",
        "passed": passed,
        "scanMethod": "eve-full-index-and-dds-headers-only",
        "scope": "caldari-battleship-model-path-or-sde-metadata",
        "anchor": {
        ...
        "minimumRecommendedFamilies": MINIMUM_RECOMMENDED_FAMILIES,
    ...
    print(f"[v16-battleship-diversity] Usable assets: {len(usable_assets)}", flush=True)
    print(
        f"[v16-battleship-diversity] Unique authored families: {len(unique_families)}",
        flush=True,
    )
    print(
        f"[v16-battleship-diversity] Four-family Stage 2: "
        f"{'SUPPORTED' if passed else 'INSUFFICIENT'}",
        flush=True,
    )
    print(f"[v16-battleship-diversity] Report: {report_path}", flush=True)
    print(f"[v16-battleship-diversity] Diagnostics ZIP: {archive_path}", flush=True)
    return (0 if passed else 2), report_path


def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Discover broader authored Caldari battleship diversity for V16 Stage 2"
    )
