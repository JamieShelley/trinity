...
class RavenTunePreviewApplication:
    # Purpose: Implement utc now for RavenTunePreviewApplication.
    # Called by: main
    ...
    # Purpose: Implement read json for RavenTunePreviewApplication.
    # Called by: main
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement resolve for RavenTunePreviewApplication.
    # Called by: main
    # Calls: No same-class helper methods.
    def _resolve(self, root: Path, value: str | Path) -> Path:
        path = Path(value)
        return path.resolve() if path.is_absolute() else (root / path).resolve()

    # Purpose: Implement parser for RavenTunePreviewApplication.
    # Called by: main
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement prepare dataset for RavenTunePreviewApplication.
    # Called by: main
    # Calls: _run
    def _prepare_dataset(self, args: argparse.Namespace, root: Path, python: str, base_config: Path, raven_config: Path) -> None:
        ...
        self._run(command, cwd=root, label=f"prepare {args.training_mode} dataset")

    # Purpose: Implement training command for RavenTunePreviewApplication.
    # Called by: main
    # Calls: No same-class helper methods.
    ...
    # Called by: main
    # Calls: No same-class helper methods.
    def _diagnostics(self, experiment_directory: Path) -> Path:
    ...
    # Purpose: Implement main for RavenTunePreviewApplication.
    # Called by: External callers and the owning workflow.
    # Calls: _diagnostics, _parser, _prepare_dataset, _read_json, _resolve, _run, _training_command, _utc_now
    def main(self, argv: list[str] | None = None) -> int:
        args = self._parser().parse_args(argv)
        root = args.repo_root.resolve()
        python = sys.executable
        base_config = self._resolve(root, args.base_config)
        raven_config = self._resolve(root, args.raven_data_config)
        ...
        train_command = self._training_command(
            args, root, python, base_config, dataset_config, result_file
        )

        requested_new = str(args.experiment).strip().upper() in {"", "NEW"}
        if requested_new:
            self._run(train_command + ["--allocate-only"], cwd=root, label="allocate experiment")
            allocation = self._read_json(result_file)
        ...
        try:
            ...
            trained_metadata = self._read_json(trained_metadata_path)
            ...
            if trained_selection != "production-final":
                diagnostics_path = self._diagnostics(experiment_directory)
                rejected = {
            ...
            if not args.skip_preview:
                ...
                self._run(preview, cwd=root, label="verify candidate provenance and launch native renderer")
...
_raven_tune_preview_application = RavenTunePreviewApplication()
_utc_now = _raven_tune_preview_application._utc_now
_run = _raven_tune_preview_application._run
_read_json = _raven_tune_preview_application._read_json
_resolve = _raven_tune_preview_application._resolve
_parser = _raven_tune_preview_application._parser
_prepare_dataset = _raven_tune_preview_application._prepare_dataset
_training_command = _raven_tune_preview_application._training_command
_diagnostics = _raven_tune_preview_application._diagnostics
main = _raven_tune_preview_application.main
