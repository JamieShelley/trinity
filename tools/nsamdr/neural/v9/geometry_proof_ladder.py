...
class GeometryProofLadderService:
    # Purpose: Implement canvas for GeometryProofLadderService.
    # Called by: proof_case_images
    # Calls: No same-class helper methods.
    def _canvas(self, draw_fn, size: int = 512) -> np.ndarray:
        ss = 4
        s = size * ss
    ...
    # Purpose: Implement proof case images for GeometryProofLadderService.
    # Called by: build_proof_case
    # Calls: _canvas
    def proof_case_images(self, size: int = 512) -> list[tuple[str, np.ndarray]]:
        ...
        for a in (1, 3, 7, 11, 19, 33, 45, 67, 83, 89):
            cases.append((f"G0_line_{a:02d}deg", self._canvas(line(a, 5), size)))
        cases.extend([
            ("G0_thin_33deg", self._canvas(line(33, 2), size)),
            ("G0_wide_19deg", self._canvas(line(19, 8), size)),
            ("G1_circle_r92", self._canvas(circle(92), size)),
            ("G1_circle_r157", self._canvas(circle(157, 4), size)),
            ("G1_ellipse_150x72", self._canvas(ellipse(150, 72, 23), size)),
            ("G1_ellipse_118x165", self._canvas(ellipse(118, 165, -17, 4), size)),
            ("G1_rounded_box", self._canvas(rounded_box, size)),
            ("G2_corner_45", self._canvas(corner(45), size)),
            ("G2_corner_90", self._canvas(corner(90), size)),
            ("G2_corner_135", self._canvas(corner(135), size)),
            ("G3_parallel_lines", self._canvas(parallel, size)),
            ("G3_concentric_ring", self._canvas(ring, size)),
            ("G3_junction", self._canvas(junction, size)),
        ])
        ...
        cases.append(("G5_degrade_blur_line33", self._canvas(line(33, 5), size)))
        cases.append(("G5_degrade_halo_circle", self._canvas(circle(118, 5), size)))
        return cases

    # Purpose: Implement pbr companions for GeometryProofLadderService.
    # Called by: build_proof_case
    # Calls: No same-class helper methods.
    def pbr_companions(self, target_rgb: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        gray = (
            target_rgb[..., 0] * 0.2126
            + target_rgb[..., 1] * 0.7152
            + target_rgb[..., 2] * 0.0722
    ...
    # Purpose: Implement teacher field for GeometryProofLadderService.
    # Called by: build_proof_case
    # Calls: No same-class helper methods.
    def teacher_field(self, target_rgb: np.ndarray, max_distance: float) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    # Purpose: Implement build proof case for GeometryProofLadderService.
    # Called by: External callers and the owning workflow.
    # Calls: pbr_companions, proof_case_images, teacher_field
    def build_proof_case(self, index: int, *, size: int, max_distance: float) -> ProofCase:
        images = self.proof_case_images(size)
        name, target_rgb_u8 = images[int(index) % len(images)]
        target_rgb = target_rgb_u8.astype(np.float32) / 255.0
        target_normal, target_material = self.pbr_companions(target_rgb)
        sdf, orientation, edge, gate = self.teacher_field(target_rgb, max_distance)
...
_geometry_proof_ladder_service = GeometryProofLadderService()
_canvas = _geometry_proof_ladder_service._canvas
proof_case_images = _geometry_proof_ladder_service.proof_case_images
pbr_companions = _geometry_proof_ladder_service.pbr_companions
teacher_field = _geometry_proof_ladder_service.teacher_field
build_proof_case = _geometry_proof_ladder_service.build_proof_case
