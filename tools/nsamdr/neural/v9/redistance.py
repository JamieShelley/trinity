...
def sdf_gradient_components(sdf_pixels: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
...
def zero_crossing_seed_distance(level_set_pixels: torch.Tensor) -> torch.Tensor:
...
def redistance_zero_contour(
    ...
    seed = zero_crossing_seed_distance(raw)
