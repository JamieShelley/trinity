...
def proof_case_primitive_target(name: str, size: int = 512) -> PrimitiveTarget:
    ...
    if m:
        ...
        p = _base_params(0.5, 0.5, angle, 5.0)
        p[4] = 1.0  # long enough to clip against the image bounds
        return PrimitiveTarget(0, p, _mask(0, 1, 2, 3, 8))
    if base == "G0_thin_33deg":
        p = _base_params(0.5, 0.5, math.radians(33.0), 2.0); p[4] = 1.0
        return PrimitiveTarget(0, p, _mask(0, 1, 2, 3, 8))
    if base == "G0_wide_19deg":
        p = _base_params(0.5, 0.5, math.radians(19.0), 8.0); p[4] = 1.0
        return PrimitiveTarget(0, p, _mask(0, 1, 2, 3, 8))
    ...
    if base in circle_specs:
        ...
        p = _base_params(0.5, 0.5, 0.0, width)
        ...
        p[4] = _axis_x_to_unit(radius, size)
        p[5] = _axis_y_to_unit(radius, size)
        return PrimitiveTarget(1, p, _mask(0, 1, 4, 5, 8))

    if base == "G1_ellipse_150x72":
        p = _base_params(0.5, 0.5, math.radians(23.0), 5.0)
        p[4] = _axis_x_to_unit(150.0, size); p[5] = _axis_y_to_unit(72.0, size)
        return PrimitiveTarget(1, p, _mask(0, 1, 2, 3, 4, 5, 8))
    if base == "G1_ellipse_118x165":
        ...
        p = _base_params(0.5, 0.5, math.radians(-17.0 + 90.0), 4.0)
        p[4] = _axis_x_to_unit(165.0, size); p[5] = _axis_y_to_unit(118.0, size)
        return PrimitiveTarget(1, p, _mask(0, 1, 2, 3, 4, 5, 8))

    if base == "G1_rounded_box":
        p = _base_params(256.0 / size, 255.0 / size, 0.0, 5.0)
        p[4] = _half_extent_x_to_unit(164.0, size)
        p[5] = _half_extent_y_to_unit(127.0, size)
        p[6] = np.float32(np.clip((46.0 / 127.0 - 0.05) / 0.45, 0.0, 1.0))
        return PrimitiveTarget(2, p, _mask(0, 1, 2, 3, 4, 5, 6, 8))

    cm = re.match(r"G2_corner_(45|90|135)$", base)
    if cm:
        included = math.radians(float(cm.group(1)))
        p = _base_params(0.5, 0.5, 0.0, 5.0)
        p[4] = _half_angle_to_unit(included * 0.5)
        p[5] = _length_to_unit(size * 0.31, size)
        return PrimitiveTarget(3, p, _mask(0, 1, 2, 3, 4, 5, 8))

    if base == "G3_parallel_lines":
        p = _base_params(0.5, 0.5, math.radians(27.0), 3.0)
        p[4] = _separation_to_unit(40.0 * math.cos(math.radians(27.0)), size)
        p[5] = _length_to_unit(size * 0.55, size)
        return PrimitiveTarget(4, p, _mask(0, 1, 2, 3, 4, 5, 8))

    if base == "G3_concentric_ring":
        p = _base_params(0.5, 0.5, 0.0, 3.0)
        p[4] = _radius_to_unit(118.0, size)
        p[5] = _radius_to_unit(132.0, size)
        return PrimitiveTarget(5, p, _mask(0, 1, 4, 5, 8))

    if base == "G3_junction":
        p = _base_params(0.5, 0.5, math.radians(15.0), 4.0)
        p[4] = _length_to_unit(170.0, size)
        return PrimitiveTarget(6, p, _mask(0, 1, 2, 3, 4, 8))
...
def random_primitive_target(
    ...
    p = _base_params(cx, cy, angle, width)
    ...
    if cls == 0:
        ...
        mask = _mask(0, 1, 2, 3, 4, 8)
    return PrimitiveTarget(cls, p.astype(np.float32), mask)


def _decode_common_torch(params: torch.Tensor, h: int, w: int, stroke_pixel_scale: float = 1.0):
    cx = params[0] * float(w)
    cy = params[1] * float(h)
    direction = torch.stack((params[2], params[3]))
    direction = direction / torch.sqrt((direction * direction).sum() + 1.0e-8)
    width = _width_from_unit_torch(params[8]) * float(stroke_pixel_scale)
    return cx, cy, direction, width


def _segment_distance_torch(xx, yy, cx, cy, direction, half_length):
    dx = xx - cx; dy = yy - cy
...
def _signed_round_box_torch(xx, yy, cx, cy, direction, half_w, half_h, radius):
    dx = xx - cx; dy = yy - cy
...
def _render_one_torch(params: torch.Tensor, cls: int, h: int, w: int, stroke_pixel_scale: float = 1.0) -> torch.Tensor:
    ...
    cx, cy, direction, full_width = _decode_common_torch(params, h, w, stroke_pixel_scale)
    ...
    if cls == 0:
        ...
        half_length = diag
        return _segment_distance_torch(xx, yy, cx, cy, direction, half_length) - half_width
    if cls == 1:
    ...
    if cls == 2:
        ...
        signed = _signed_round_box_torch(xx, yy, cx, cy, direction, half_w, half_h, radius)
    ...
    if cls == 3:
        ...
        return torch.minimum(
            _segment_distance_torch(xx, yy, cx + d1[0] * length * 0.5, cy + d1[1] * length * 0.5, d1, length * 0.5),
            _segment_distance_torch(xx, yy, cx + d2[0] * length * 0.5, cy + d2[1] * length * 0.5, d2, length * 0.5),
        ) - half_width
    if cls == 4:
        separation = (0.02 + 0.18 * params[4]) * min_dim
        half_length = (0.15 + 0.45 * params[5]) * min_dim
        normal = torch.stack((-direction[1], direction[0]))
        d0 = _segment_distance_torch(xx, yy, cx + normal[0] * separation * 0.5, cy + normal[1] * separation * 0.5, direction, half_length)
        d1 = _segment_distance_torch(xx, yy, cx - normal[0] * separation * 0.5, cy - normal[1] * separation * 0.5, direction, half_length)
    ...
    for delta in angles:
        ...
        distances.append(_segment_distance_torch(xx, yy, cx + d[0] * length * 0.5, cy + d[1] * length * 0.5, d, length * 0.5))
...
def render_parametric_sdf_torch(
    ...
    for i in range(int(params.shape[0])):
        fields.append(_render_one_torch(
...
def _render_one_numpy(params: np.ndarray, cls: int, size: int) -> np.ndarray:
    p = torch.from_numpy(np.asarray(params, dtype=np.float32))
    with torch.no_grad():
        field = _render_one_torch(p, int(cls), int(size), int(size)).cpu().numpy()
    return np.ascontiguousarray(field.astype(np.float32))


def render_parametric_sdf_numpy(target: PrimitiveTarget, size: int) -> np.ndarray:
    return _render_one_numpy(target.params, target.class_index, size)
    ...
    *,
    ...
    for sample_index in range(int(inputs_lr.shape[0])):
        ...
        def add_seed(cls: int, q: torch.Tensor) -> None:
            q = q.clone()
            q[:, cls, 8] = _width_to_unit(canonical_width[cls])
            seeds.append(q)
            classes.append(cls)
        ...
        with torch.enable_grad():
            for _ in range(max(1, int(steps))):
                ...
                sdf_fit = render_parametric_sdf_torch(
