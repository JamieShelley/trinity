...
def set_active_evolution_genome(value: dict[str, float] | None) -> None:
...
def active_evolution_genome() -> dict[str, float]:
    return dict(_ACTIVE_EVOLUTION_GENOME)


def _require_current_v11_instance(instance: Any) -> None:
...
def _geometry_init(self: Any, config: Any) -> None:
    ...
    _require_current_v11_instance(self)
...
def _geometry_forward(self: Any, inputs: torch.Tensor) -> dict[str, torch.Tensor]:
    context = _geometry_encode(self, inputs)
    aux = context["aux"]
...
def _validate_local_architecture(contract: dict[str, object]) -> None:
...
def install_local_boundary_model_contract() -> None:
...
def install_local_boundary_training_contract(training_module: ModuleType) -> None:
