...
def _resize_for_analysis(image: np.ndarray, max_dim: int) -> tuple[np.ndarray, float]:
    h, w = image.shape[:2]
...
def _edge_mask(gray: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    _gx, _gy, mag = _gradient(gray)
...
def _fit_straightness(edge: np.ndarray, mag: np.ndarray) -> dict[str, float] | None:
...
def _curvature_noise(edge: np.ndarray) -> float | None:
...
def _ringing_proxy(gray: np.ndarray, edge: np.ndarray) -> float:
...
def _topology_components(edge: np.ndarray) -> int:
    count, _labels = cv2.connectedComponents(edge.astype(np.uint8), connectivity=8)
    return max(0, int(count) - 1)


def _symmetric_edge_chamfer(edge_a: np.ndarray, edge_b: np.ndarray) -> float:
...
def _candidate_patches(edge: np.ndarray, mag: np.ndarray, patch: int, stride: int, limit: int = 48) -> list[tuple[int, int, int, int, float]]:
    h, w = edge.shape
...
def _flow_metrics(flow: np.ndarray | None, edge: np.ndarray, gx: np.ndarray, gy: np.ndarray, scale: float) -> dict[str, Any]:
    if flow is None:
...
def _boundary_gate_metrics(
    ...
    g = np.clip(g, 0.0, 1.0)
    edge_band = _dilate(edge, 5)
    off_edge = ~_dilate(edge, 10)
...
def _patch_metrics(gray: np.ndarray, edge: np.ndarray, mag: np.ndarray, region: tuple[int, int, int, int, float]) -> dict[str, Any]:
    x, y, w, h, energy = region
    g = gray[y:y + h, x:x + w]
    e = edge[y:y + h, x:x + w]
    m = mag[y:y + h, x:x + w]
    straight = _fit_straightness(e, m)
    general_width = _general_edge_width_proxy(g, e)
    ...
    return {
        ...
        "curvatureNoise": _curvature_noise(e),
        "ringingProxy": _ringing_proxy(g, e),
        "componentCount": _topology_components(e),
    }
...
def _synthetic_critic_batch(batch: int, size: int, device, generator) -> tuple[Any, Any, Any]:
    import torch
...
def ensure_geometry_critic(checkpoint: Path, *, device: str = "cpu", steps: int = 180, seed: int = 98173) -> dict[str, Any]:
    import torch
    import torch.nn.functional as F

    checkpoint = checkpoint.resolve()
    checkpoint.parent.mkdir(parents=True, exist_ok=True)
    GeometryCritic = _critic_classes()
    ...
    for _ in range(max(40, int(steps))):
        a, b, label = _synthetic_critic_batch(32, 64, dev, gen)
...
def _critic_scores(checkpoint: Path, baseline: np.ndarray, candidate: np.ndarray, regions: Iterable[tuple[int,int,int,int,float]], device: str) -> dict[str, Any]:
    import torch
    import torch.nn.functional as F
    GeometryCritic = _critic_classes()
    try:
...
def _render_evidence(path: Path, baseline: np.ndarray, candidate: np.ndarray, b_edge: np.ndarray, c_edge: np.ndarray,
...
def audit_pair(
    ...
    *,
    ...
    baseline0, scale = _resize_for_analysis(baseline_image, options.max_analysis_dimension)
    ...
    bg = _gray(baseline0); cg = _gray(candidate0)
    be, bmag = _edge_mask(bg); ce, cmag = _edge_mask(cg)
    bgx,bgy,_ = _gradient(bg)
    regions = _candidate_patches(be, bmag, options.patch_size, options.patch_stride)
    rows: list[dict[str, Any]] = []
    for index, region in enumerate(regions):
        bm = _patch_metrics(bg, be, bmag, region)
        cm = _patch_metrics(cg, ce, cmag, region)
        straight_imp = _relative_improvement(bm["straightnessRms"], cm["straightnessRms"])
        curvature_imp = _relative_improvement(bm["curvatureNoise"], cm["curvatureNoise"])
        width_delta = _metric_delta(bm["edgeWidthProxy"], cm["edgeWidthProxy"])
        ring_imp = _relative_improvement(bm["ringingProxy"], cm["ringingProxy"])
    ...
    topology_before = _topology_components(be)
    topology_after = _topology_components(ce)
    ringing_before = _ringing_proxy(bg, be)
    ringing_after = _ringing_proxy(cg, ce)
    flow_report = _flow_metrics(flow, be, bgx, bgy, scale)
    boundary_report = _boundary_gate_metrics(gate, be, scale)
    ...
    if target_image is not None:
        ...
        tg = _gray(target0); te,_ = _edge_mask(tg)
        cb = _symmetric_edge_chamfer(be, te); cc = _symmetric_edge_chamfer(ce, te)
        mae_b = float(np.mean(np.abs(bg-tg))); mae_c = float(np.mean(np.abs(cg-tg)))
        exact = {
            "available": True,
            "edgeChamferBefore": cb,
            "edgeChamferAfter": cc,
            "edgeChamferImprovement": _relative_improvement(cb, cc),
            "lumaMaeBefore": mae_b,
            "lumaMaeAfter": mae_c,
            "lumaMaeImprovement": _relative_improvement(mae_b, mae_c),
        }
    ...
    if options.critic_mode != "off" and critic_checkpoint is not None:
        calibration = ensure_geometry_critic(critic_checkpoint, device=critic_device)
        critic.update(calibration)
        scores = _critic_scores(critic_checkpoint, bg, cg, regions[:24], critic_device)
    ...
    if rows and count:
        ...
        for rank, row in enumerate(selected_rows):
            ...
            _render_evidence(path, baseline0, candidate0, be, ce, regions[idx], flow, gate, f"{source_name} | {kind} | region {idx}", row)
...
def write_audit_bundle(output_dir: Path, reports: list[dict[str, Any]]) -> dict[str, Any]:
