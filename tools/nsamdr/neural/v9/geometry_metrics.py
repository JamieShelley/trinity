...
class GeometryMetrics:
    # Purpose: Implement gray rgb for GeometryMetrics.
    # Called by: profile_width_rms_pixels, synthetic_region_boundary, synthetic_region_components
    # Calls: No same-class helper methods.
    def _gray_rgb(self, image_rgb: np.ndarray) -> np.ndarray:
    ...
    # Purpose: Implement synthetic region components for GeometryMetrics.
    # Called by: topology_mismatch
    # Calls: _gray_rgb
    def synthetic_region_components(self, image_rgb: np.ndarray, target_rgb: np.ndarray) -> int:
        gray = self._gray_rgb(image_rgb)
        target_gray = self._gray_rgb(target_rgb)
    ...
    # Purpose: Implement topology mismatch for GeometryMetrics.
    # Called by: External callers and the owning workflow.
    # Calls: synthetic_region_components
    def topology_mismatch(self, image_rgb: np.ndarray, target_rgb: np.ndarray) -> float:
        return float(
            self.synthetic_region_components(image_rgb, target_rgb)
            != self.synthetic_region_components(target_rgb, target_rgb)
        )

    # Purpose: Implement synthetic region boundary for GeometryMetrics.
    # Called by: synthetic_region_chamfer
    # Calls: _gray_rgb
    def synthetic_region_boundary(self, image_rgb: np.ndarray, target_rgb: np.ndarray) -> np.ndarray:
    ...
    # Purpose: Implement synthetic region chamfer for GeometryMetrics.
    # Called by: synthetic_region_chamfer_improvement
    # Calls: synthetic_region_boundary
    def synthetic_region_chamfer(self, image_rgb: np.ndarray, target_rgb: np.ndarray) -> float:
        candidate = self.synthetic_region_boundary(image_rgb, target_rgb)
        target = self.synthetic_region_boundary(target_rgb, target_rgb)
    ...
    # Purpose: Implement synthetic region chamfer improvement for GeometryMetrics.
    # Called by: External callers and the owning workflow.
    # Calls: synthetic_region_chamfer
    def synthetic_region_chamfer_improvement(
        ...
        before = self.synthetic_region_chamfer(baseline_rgb, target_rgb)
        after = self.synthetic_region_chamfer(candidate_rgb, target_rgb)
    ...
    # Purpose: Implement profile width rms pixels for GeometryMetrics.
    # Called by: profile_width_ratio
    # Calls: _gray_rgb
    ...
    # Purpose: Implement profile width ratio for GeometryMetrics.
    # Called by: External callers and the owning workflow.
    # Calls: profile_width_rms_pixels
    def profile_width_ratio(
        ...
        candidate = self.profile_width_rms_pixels(image_rgb, target_sdf_pixels)
        target = self.profile_width_rms_pixels(target_rgb, target_sdf_pixels)
        if not np.isfinite(candidate) or not np.isfinite(target) or target <= 1.0e-8:
            return float("inf")
        return float(candidate / target)

    # Purpose: Implement zero crossing points for GeometryMetrics.
    # Called by: circle_radial_roughness_pixels, line_perpendicular_jitter_pixels, line_staircase_energy_pixels, zero_contour_distance
    # Calls: No same-class helper methods.
    def zero_crossing_points(self, field: np.ndarray) -> np.ndarray:
    ...
    # Purpose: Implement nearest distances for GeometryMetrics.
    # Called by: zero_contour_distance
    # Calls: No same-class helper methods.
    def _nearest_distances(self, query: np.ndarray, reference: np.ndarray, chunk: int = 256) -> np.ndarray:
    ...
    # Purpose: Implement binary topology signature for GeometryMetrics.
    # Called by: sdf_topology_mismatch, sdf_topology_signature
    # Calls: No same-class helper methods.
    def _binary_topology_signature(self, mask: np.ndarray, min_component_pixels: int = 8) -> tuple[int, int]:
    ...
    # Purpose: Implement sdf topology signature for GeometryMetrics.
    # Called by: sdf_topology_mismatch
    # Calls: _binary_topology_signature
    def sdf_topology_signature(
        ...
        *,
        ...
        return self._binary_topology_signature(aligned, min_component_pixels)

    # Purpose: Implement sdf topology mismatch for GeometryMetrics.
    # Called by: External callers and the owning workflow.
    # Calls: _binary_topology_signature, sdf_topology_signature
    def sdf_topology_mismatch(
        ...
        *,
        ...
        predicted_signature = self.sdf_topology_signature(
            field, target_field, min_component_pixels=min_component_pixels
        )
        target = np.asarray(target_field, dtype=np.float32)
        if target.ndim == 3:
            target = target[..., 0]
        target_signature = self._binary_topology_signature(
            target < 0.0, min_component_pixels
        )
        return float(predicted_signature != target_signature)

    # Purpose: Implement zero contour distance for GeometryMetrics.
    # Called by: External callers and the owning workflow.
    # Calls: _nearest_distances, zero_crossing_points
    def zero_contour_distance(self, predicted_field: np.ndarray, target_field: np.ndarray) -> dict[str, float]:
        ...
        p_to_t = self._nearest_distances(predicted, target)
        t_to_p = self._nearest_distances(target, predicted)
    ...
    # Purpose: Implement branch centres 1d for GeometryMetrics.
    # Called by: circle_radial_roughness_pixels, line_perpendicular_jitter_pixels, line_staircase_energy_pixels
    # Calls: No same-class helper methods.
    def _branch_centres_1d(self, values: np.ndarray, *, minimum_separation: float = 0.75) -> np.ndarray:
    ...
    # Purpose: Implement branch detrended rms for GeometryMetrics.
    # Called by: circle_radial_roughness_pixels, line_perpendicular_jitter_pixels
    # Calls: No same-class helper methods.
    def _branch_detrended_rms(self, values: np.ndarray, centres: np.ndarray) -> float:
        data = np.asarray(values, dtype=np.float32).reshape(-1)
    ...
    # Purpose: Implement line perpendicular jitter pixels for GeometryMetrics.
    # Called by: External callers and the owning workflow.
    # Calls: _branch_centres_1d, _branch_detrended_rms, zero_crossing_points
    def line_perpendicular_jitter_pixels(
        ...
        centres = self._branch_centres_1d(target_signed)
        return self._branch_detrended_rms(predicted_signed, centres)

    # Purpose: Implement circle radial roughness pixels for GeometryMetrics.
    # Called by: External callers and the owning workflow.
    # Calls: _branch_centres_1d, _branch_detrended_rms, zero_crossing_points
    def circle_radial_roughness_pixels(
        ...
        centres = self._branch_centres_1d(target_radius)
        return self._branch_detrended_rms(predicted_radius, centres)

    # Purpose: Implement line staircase energy pixels for GeometryMetrics.
    # Called by: line_staircase_recovery
    # Calls: _branch_centres_1d, zero_crossing_points
    def line_staircase_energy_pixels(
        ...
        *,
        ...
        branch_centres = self._branch_centres_1d(target_n)
...
_geometry_metrics = GeometryMetrics()
_gray_rgb = _geometry_metrics._gray_rgb
synthetic_region_components = _geometry_metrics.synthetic_region_components
topology_mismatch = _geometry_metrics.topology_mismatch
synthetic_region_boundary = _geometry_metrics.synthetic_region_boundary
synthetic_region_chamfer = _geometry_metrics.synthetic_region_chamfer
synthetic_region_chamfer_improvement = _geometry_metrics.synthetic_region_chamfer_improvement
profile_width_rms_pixels = _geometry_metrics.profile_width_rms_pixels
profile_width_ratio = _geometry_metrics.profile_width_ratio
zero_crossing_points = _geometry_metrics.zero_crossing_points
_nearest_distances = _geometry_metrics._nearest_distances
_binary_topology_signature = _geometry_metrics._binary_topology_signature
sdf_topology_signature = _geometry_metrics.sdf_topology_signature
sdf_topology_mismatch = _geometry_metrics.sdf_topology_mismatch
zero_contour_distance = _geometry_metrics.zero_contour_distance
_branch_centres_1d = _geometry_metrics._branch_centres_1d
_branch_detrended_rms = _geometry_metrics._branch_detrended_rms
line_perpendicular_jitter_pixels = _geometry_metrics.line_perpendicular_jitter_pixels
circle_radial_roughness_pixels = _geometry_metrics.circle_radial_roughness_pixels
line_staircase_energy_pixels = _geometry_metrics.line_staircase_energy_pixels
line_staircase_recovery = _geometry_metrics.line_staircase_recovery
