...
def _subpixel_target_stack(value: torch.Tensor, grid_n: int = 3) -> torch.Tensor:
...
def _coverage_profile_width(
    ...
    *,
    ...
    """
    grad = gradient_magnitude(coverage.float())
...
def _topology_sign_margin_loss(
    ...
    *,
    ...
    mean_violation = _sum_fp32(violation * active) / _sum_fp32(active).clamp_min(1.0)
...
def _directional_derivatives(value: torch.Tensor, tangent: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    scalar = value.float().mean(dim=1, keepdim=True)
...
def _projected_view_seam_loss(
    ...
    for degrees, sx, sy in ((23.0, 0.78, 1.0), (-41.0, 1.0, 0.72)):
        ...
        gp = gradient_magnitude(wp.mean(dim=1, keepdim=True))
        gt = gradient_magnitude(wt.mean(dim=1, keepdim=True))
        total = total + _weighted_mean((gp - gt).abs(), 0.05 + 1.95 * we)
    return total * 0.5
...
def compute_losses(
    ...
    if stroke_active is not None and float(stroke_active.detach().float().mean().item()) > 0.5:
        ...
        losses["stroke_render"] = _weighted_mean(
            F.smooth_l1_loss(
        ...
        losses["stroke_width_mean_pixels"] = _weighted_mean(half_width[:, 0:1].detach(), ridge.detach())
    else:
    ...
    if (
        ...
        losses["primitive_render"] = _weighted_mean(
    ...
    losses["sdf_raw_surface"] = _balanced_metric_band_mean(
    ...
    losses["sdf_delta_surface"] = _balanced_metric_band_mean(
    ...
    losses["sdf_delta_tangent"] = _weighted_mean(
        F.smooth_l1_loss(pred_delta_dt, target_delta_dt, beta=0.20, reduction="none"),
        delta_shape_weight,
    )
    pred_delta_lap = laplacian_tensor(predicted_delta_pixels)
    target_delta_lap = laplacian_tensor(target_delta_pixels.detach()).detach()
    losses["sdf_delta_laplacian"] = _weighted_mean(
    ...
    if predicted_transport_control is not None and predicted_transport_hr is not None:
        ...
        projected_transport, transport_confidence, projected_transport_hr, transport_confidence_hr = (
            _project_contour_vector_to_control_lattice(
                target_transport_hr,
                offset_weight,
        ...
        losses["contour_transport"] = _weighted_mean(
    ...
    if predicted_dilation_control is not None and predicted_dilation_hr is not None:
        ...
        projected_dilation, dilation_confidence, projected_dilation_hr, dilation_confidence_hr = (
            _project_contour_offset_to_source_lattice(
                target_dilation_hr,
        ...
        losses["contour_dilation"] = _weighted_mean(
            F.smooth_l1_loss(
    ...
    losses["contour_normal_offset"] = _weighted_mean(
        F.smooth_l1_loss(
    ...
    if predicted_transport_hr is not None:
        ...
        losses["contour_transport_fold"] = _weighted_mean(
            fold_penalty,
    ...
    losses["contour_soft_coverage"] = _weighted_mean(
    ...
    if implicit_samples is not None and implicit_samples.shape[1] == 9:
        ...
        target_samples = _subpixel_target_stack(target_sdf_pixels.detach(), 3)
        sample_weight = (
            0.15 + 1.85 * torch.exp(-target_samples.abs() / 4.0)
        ).detach()
        losses["implicit_subpixel_surface"] = _weighted_mean(
        ...
        losses["implicit_subpixel_gradient"] = _weighted_mean(
        ...
        losses["implicit_subpixel_eikonal"] = _weighted_mean(
    ...
    if oracle_patch_sdf is not None and oracle_patch_cov is not None:
        ...
        losses["oracle_patch_sdf"] = _weighted_mean(patch_surface, patch_weight)
        ...
        losses["oracle_patch_sign"] = _weighted_mean(
        ...
        losses["oracle_patch_coverage"] = _weighted_mean(
        ...
        losses["oracle_patch_coverage_bce"] = _weighted_mean(
        ...
        losses["oracle_patch_overlap_consistency"] = _weighted_mean(
            F.smooth_l1_loss(
        ...
        losses["oracle_patch_gradient"] = _weighted_mean(
        ...
        if direct_cov is not None:
            ...
            losses["oracle_coverage_aggregate"] = _weighted_mean(
    ...
    if (spline_control is not None and spline_source_control is not None
        ...
        losses["spline_graph_topology_control"] = _weighted_mean(
        ...
        losses["spline_graph_topology_sign"] = _weighted_mean(
        ...
        losses["spline_graph_sdf"] = _weighted_mean(
        ...
        losses["spline_graph_gradient"] = _weighted_mean(
        ...
        losses["spline_graph_eikonal"] = _weighted_mean((snorm - 1.0).abs(), field_weight)
        ...
        losses["spline_metric_offset"] = _weighted_mean(
        ...
        losses["spline_metric_eikonal_near"] = _weighted_mean(
        ...
        if metric_scale is not None and metric_bias is not None:
            ...
            losses["spline_metric_scale_regularization"] = _weighted_mean(
                (metric_scale.float() - 1.0).abs(), reg_weight
            )
            losses["spline_metric_bias_regularization"] = _weighted_mean(
        ...
        losses["spline_graph_curvature"] = _weighted_mean(
            (laplacian_tensor(predicted_sdf_pixels) - laplacian_tensor(target_detached)).abs(),
            field_weight,
        )
    ...
    if primitive_anchor is not None:
        ...
        losses["parametric_anchor"] = _weighted_mean(
    ...
    if predicted_transport_hr is not None:
        ...
        losses["contour_transport_smoothness"] = _weighted_mean(
            transport_grad_error,
    ...
    if predicted_dilation_hr is not None:
        ...
        losses["contour_dilation_smoothness"] = _weighted_mean(
            0.5 * (
    ...
    losses["contour_offset_rms_pixels"] = torch.sqrt(
        _weighted_mean(predicted_normal_offset_pixels.square(), offset_weight).clamp_min(0.0)
        + 1.0e-8
    ).detach()
    ...
    losses["sdf_improvement_regret"] = _balanced_metric_band_mean(
        improvement_regret * need_weight, target_sdf_pixels, metric_band, near_pixels=3.0
    )
    losses["sdf_source_surface"] = _balanced_metric_band_mean(
        source_abs_error, target_sdf_pixels, metric_band, near_pixels=3.0
    ).detach()
    losses["geometry_need_mean"] = _mean_fp32(geometry_need.float()).detach()
    ...
    losses["sdf_sign"] = _balanced_sign_mean(
        sign_error, target_sdf_pixels, metric_band, near_pixels=2.0
    )
    losses["sdf_topology_sign"] = _topology_sign_margin_loss(
        predicted_sdf_pixels,
        target_sdf_pixels,
    ...
    redistanced_eikonal = _weighted_mean(
        (metric_pred_grad_norm - 1.0).abs(), metric_mask
    ).detach()
    redistanced_near_eikonal = _weighted_mean(
    ...
    losses["boundary_sdf_raw_zero"] = _weighted_mean(
        F.smooth_l1_loss(
            predicted_sdf_pixels,
    ...
    losses["sdf_raw_zero_rms_levelset_units"] = torch.sqrt(
        _weighted_mean(raw_zero_sq, zero_mask).clamp_min(0.0) + 1.0e-12
    )
    losses["sdf_zero_rms_pixels"] = torch.sqrt(
        _weighted_mean(metric_zero_sq, zero_mask).clamp_min(0.0) + 1.0e-12
    )
    losses["sdf_grad_norm_mean"] = _weighted_mean(pred_grad_norm, metric_mask)
    redistanced_grad_norm_mean = _weighted_mean(
    ...
    losses["sdf_metricization_denominator_mean"] = _weighted_mean(
    ...
    if seam_tangent is not None and seam_authority is not None and seam_coherence is not None:
        ...
        target_axial = _axial_encoding(target_tangent)
        seam_axial = _axial_encoding(seam_tangent_unit)
        seam_mask = (0.05 + target_edge.float() * 1.95) * (0.25 + seam_coherence.float().detach() * 0.75)
        losses["seam_directional"] = _weighted_mean(
            1.0 - (target_axial * seam_axial).sum(dim=1, keepdim=True).clamp(-1.0, 1.0), seam_mask.detach()
        )
        pred_t, pred_n = _directional_derivatives(reconstructed_albedo, target_tangent)
        tgt_t, tgt_n = _directional_derivatives(target_albedo, target_tangent)
        losses["seam_tangent_smoothness"] = _weighted_mean(
            (pred_t - tgt_t).abs(), seam_mask.detach()
        )
        losses["seam_normal_profile"] = _weighted_mean(
            (pred_n.abs() - tgt_n.abs()).abs(), seam_mask.detach()
        )
        off_seam = (1.0 - target_edge.float()).clamp(0.0, 1.0)
        losses["seam_authority_regularization"] = _weighted_mean(
        ...
        losses["seam_reconstruction"] = _weighted_mean(
        ...
        if phase_delta is not None:
            ...
            losses["seam_phase_residual"] = _weighted_mean(
                (phase_delta.float() - target_delta).abs(), phase_weight
            )
        else:
            losses["seam_phase_residual"] = zero
        with torch.no_grad():
            base_err = _weighted_mean((seam_source_albedo.float() - target_albedo.float()).abs(), seam_weight)
            cand_err = _weighted_mean((reconstructed_albedo.float() - target_albedo.float()).abs(), seam_weight)
        ...
        if phase in {"sdf-proof", "seam-proof"}:
            losses["seam_projected_view"] = _projected_view_seam_loss(
    ...
    if teacher_albedo is not None:
        ...
        losses["sdf_teacher_render"] = _weighted_mean(
        ...
        teacher_grad = gradient_magnitude(teacher_gray)
        recon_grad_teacher = gradient_magnitude(recon_gray_teacher)
        losses["sdf_teacher_gradient"] = _weighted_mean(
            (recon_grad_teacher - teacher_grad).abs(),
            teacher_band,
        )
        teacher_edge_proxy = _normalise_edge(teacher_grad)
        recon_teacher_edge_proxy = _normalise_edge(recon_grad_teacher)
        losses["sdf_teacher_profile"] = _weighted_mean(
        ...
        baseline_teacher_error = _weighted_mean(
            (baseline_albedo.float() - teacher_albedo).abs().mean(dim=1, keepdim=True),
            teacher_band,
        )
        predicted_teacher_error = _weighted_mean(
            teacher_albedo_error,
            teacher_band,
        )
    ...
    if refined_coverage is not None:
        ...
        losses["boundary_specialist_coverage"] = _weighted_mean(
            (pred_cov - target_cov).abs(), specialist_band
        )
        bce = F.binary_cross_entropy(pred_cov, target_cov, reduction="none")
        losses["boundary_specialist_coverage_bce"] = _weighted_mean(bce, specialist_band)
        pred_gx, pred_gy = sobel_tensor(pred_cov)
        target_gx, target_gy = sobel_tensor(target_cov)
        losses["boundary_specialist_coverage_gradient"] = _weighted_mean(
            (pred_gx - target_gx).abs() + (pred_gy - target_gy).abs(),
            specialist_band,
        )
        pred_width = _coverage_profile_width(pred_cov, target_sdf_pixels, band_pixels=6.0)
        target_width = _coverage_profile_width(target_cov, target_sdf_pixels, band_pixels=6.0).detach()
    ...
    baseline_boundary_mae = _weighted_mean(
    ...
    losses["boundary_candidate_gain"] = _weighted_mean(
        candidate_gain.detach(), (0.05 + target_edge_band).detach()
    )
    losses["boundary_candidate_win_fraction"] = _weighted_mean(
    ...
    detail_base_error = _mean_fp32(detail_base_error_map)
    detail_final_error = _mean_fp32(detail_final_error_map)
    ...
    losses["detail_win_fraction"] = _mean_fp32((detail_final_error_map < detail_base_error_map).float()).detach()
    losses["detail_regression_fraction"] = _mean_fp32(
        (detail_final_error_map > detail_base_error_map + 0.002).float()
    ).detach()

    detail_base_grad_error = _mean_fp32((
        gradient_magnitude(base_albedo_detached) - gradient_magnitude(target_albedo.float())
    ).abs())
    detail_final_grad_error = _mean_fp32((
        gradient_magnitude(detail_candidate_albedo) - gradient_magnitude(target_albedo.float())
    ).abs())
    ...
    final_output_error = _mean_fp32(final_output_error_map)
    ...
    losses["final_win_fraction"] = _mean_fp32(
        (final_output_error_map < detail_base_error_map).float()
    ).detach()
    losses["final_regression_fraction"] = _mean_fp32(
    ...
    losses["detail_contrast"] = charbonnier(
        _local_contrast(detail_candidate_albedo), _local_contrast(target_albedo)
    )

    final_albedo_edge = gradient_magnitude(detail_candidate_albedo.mean(dim=1, keepdim=True))
    final_normal_edge = gradient_magnitude(detail_candidate_normal).amax(dim=1, keepdim=True)
    final_material_edge = gradient_magnitude(detail_candidate_material).amax(dim=1, keepdim=True)
    target_albedo_edge = gradient_magnitude(target_albedo.float().mean(dim=1, keepdim=True))
    target_normal_edge = gradient_magnitude(target_normal.float()).amax(dim=1, keepdim=True)
    ...
    losses["detail_candidate_albedo"] = charbonnier(
        detail_candidate_albedo, target_albedo.float()
    )
    losses["detail_candidate_albedo_gradient"] = gradient_loss(
        detail_candidate_albedo, target_albedo.float()
    )
    losses["detail_candidate_normal"] = normal_cosine_loss(
        detail_candidate_normal, target_normal.float()
    )
    losses["detail_candidate_normal_gradient"] = gradient_loss(
        detail_candidate_normal, target_normal.float()
    )
    losses["detail_candidate_roughness"] = masked_mean(
        torch.sqrt((detail_candidate_material[:, 2:3] - target_roughness.float()).square() + 1.0e-6),
        auxiliary_valid,
    )
    losses["detail_candidate_emissive"] = masked_mean(
    ...
    losses["detail_candidate_material"] = masked_mean(
        F.cross_entropy(
    ...
    target_material_edge = gradient_magnitude(target_material_rgb).amax(dim=1, keepdim=True)
    losses["detail_cross_map"] = (
        charbonnier(final_albedo_edge.clamp(0.0, 1.0), target_albedo_edge.clamp(0.0, 1.0))
        + 0.5 * charbonnier(final_normal_edge.clamp(0.0, 1.0), target_normal_edge.clamp(0.0, 1.0))
        + 0.35 * charbonnier(final_material_edge.clamp(0.0, 1.0), target_material_edge.clamp(0.0, 1.0))
    )
