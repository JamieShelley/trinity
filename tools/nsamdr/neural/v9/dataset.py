...
def _block_view(array: np.ndarray) -> tuple[np.ndarray, tuple[int, int]]:
...
def _unblock_view(blocks: np.ndarray, shape: tuple[int, int]) -> np.ndarray:
...
def _bc1_palette_like(array: np.ndarray) -> np.ndarray:
    """Vectorised BC1 RGB endpoint/palette approximation on real 4x4 blocks."""
    blocks, shape = _block_view(np.clip(array, 0.0, 1.0))
    ...
    return np.clip(_unblock_view(decoded, shape), 0.0, 1.0).astype(np.float32)


def _bc4_palette_like(channel: np.ndarray) -> np.ndarray:
    """BC4-like scalar endpoint/interpolant approximation."""
    blocks, shape = _block_view(np.clip(channel, 0.0, 1.0))
...
def _codec_kind(format_name: str | None, role: str) -> str:
...
def _dds_codec_like(
    ...
    kind = _codec_kind(format_name, role)
    if kind == "bc5":
        encoded = np.clip(padded * 0.5 + 0.5, 0.0, 1.0) if role == "normal" else np.clip(padded, 0.0, 1.0)
        channels = [_bc4_palette_like(encoded[..., i:i + 1]) for i in range(min(2, encoded.shape[-1]))]
        decoded = np.concatenate(channels + ([encoded[..., 2:]] if encoded.shape[-1] > 2 else []), axis=-1)
        if role == "normal":
            decoded = (decoded - 0.5) * 2.0
            decoded = _renormalize_normal(decoded[..., :2])
    elif kind == "bc3":
        decoded = _bc1_palette_like(np.clip(padded, 0.0, 1.0))
    else:
        decoded = _bc1_palette_like(np.clip(padded, 0.0, 1.0))
...
def degrade_physical_maps(
    ...
    if bool(getattr(config, "dds_codec_degradation_enabled", True)) and rng.random() < float(getattr(config, "dds_codec_probability", 0.90)):
        ...
        albedo = _dds_codec_like(albedo, formats.get("albedo"), "albedo", blend, rng)
        normal = _dds_codec_like(normal, formats.get("normal"), "normal", min(1.0, blend * 0.85), rng)
        material = _dds_codec_like(material, formats.get("material"), "material", min(1.0, blend * 0.75), rng)
...
def _synthetic_geometry_sample(
    ...
    *,
    ...
    """
    signed_distance, _kind = _analytic_shape_distance(
