...
class TrainingService:
    # Purpose: Implement status for TrainingService.
    ...
    # Purpose: Implement oracle render match numpy for TrainingService.
    # Called by: _validate
    # Calls: _render_profile_metrics_numpy
    def _oracle_render_match_numpy(
        ...
        candidate_profile = self._render_profile_metrics_numpy(candidate, target_rgb, sdf)
        oracle_profile = self._render_profile_metrics_numpy(oracle, target_rgb, sdf)
    ...
    # Purpose: Implement config hash for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    def _config_hash(self, config: V9Config) -> str:
    ...
    # Purpose: Implement phase for epoch for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    def _phase_for_epoch(self, epoch: int, config: V9Config) -> str:
    ...
    # Purpose: Implement parametric b1b substage for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement phase lr for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    def _phase_lr(self, phase: str, config: V9Config, epoch: int | None = None) -> float:
        base = {
    ...
    # Calls: No same-class helper methods.
    def _atomic_torch_save(self, payload: object, path: Path) -> None:
    ...
    # Purpose: Implement atomic json for TrainingService.
    # Called by: _abort_nonfinite, train_v9
    # Calls: No same-class helper methods.
    def _atomic_json(self, payload: object, path: Path) -> None:
        path.parent.mkdir(parents=True, exist_ok=True)
    ...
    # Purpose: Implement move batch for TrainingService.
    # Called by: _iter_batches, _run_final_qualification
    # Calls: No same-class helper methods.
    def _move_batch(
        self,
    ...
    # Purpose: Implement finite tensor for TrainingService.
    # Called by: _bad_gradient_names, _bad_parameter_names, _validate, train_v9
    # Calls: No same-class helper methods.
    def _finite_tensor(self, value: torch.Tensor) -> bool:
        return bool(torch.isfinite(value.detach()).all().item())

    # Purpose: Implement bad gradient names for TrainingService.
    # Called by: _abort_nonfinite, train_v9
    # Calls: _finite_tensor
    def _bad_gradient_names(self, model: torch.nn.Module, limit: int = 24) -> list[str]:
        ...
        for name, parameter in model.named_parameters():
            if parameter.grad is not None and not self._finite_tensor(parameter.grad):
                names.append(name)
                if len(names) >= limit:
                    break
        return names

    # Purpose: Implement bad parameter names for TrainingService.
    # Called by: _abort_nonfinite, train_v9
    # Calls: _finite_tensor
    def _bad_parameter_names(self, model: torch.nn.Module, limit: int = 24) -> list[str]:
        names: list[str] = []
        for name, parameter in model.named_parameters():
            if not self._finite_tensor(parameter):
                names.append(name)
                if len(names) >= limit:
                    break
        return names

    # Purpose: Implement parameters are finite for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    def _parameters_are_finite(self, model: torch.nn.Module) -> bool:
    ...
    # Purpose: Implement abort nonfinite for TrainingService.
    # Called by: _validate, train_v9
    # Calls: _atomic_json, _atomic_torch_save, _bad_gradient_names, _bad_parameter_names
    def _abort_nonfinite(
        ...
        *,
        ...
        report = {
            ...
            "stage": stage,
            "details": details,
            "badGradients": self._bad_gradient_names(model),
            "badParameters": self._bad_parameter_names(model),
        }
        self._atomic_json(report, report_path)
        ...
        self._atomic_torch_save(payload, tensor_path)
    ...
    # Purpose: Implement capture rng for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement rng byte tensor for TrainingService.
    # Called by: _restore_rng
    # Calls: No same-class helper methods.
    def _rng_byte_tensor(self, value: Any) -> torch.Tensor:
    ...
    # Purpose: Implement restore rng for TrainingService.
    # Called by: train_v9
    # Calls: _rng_byte_tensor
    def _restore_rng(self, payload: dict[str, Any]) -> None:
        ...
        torch.set_rng_state(self._rng_byte_tensor(payload["torch"]))
        ...
        if torch.cuda.is_available() and "cuda" in payload:
            ...
            cuda_states = [self._rng_byte_tensor(state) for state in raw_cuda_states]
    ...
    # Purpose: Implement save contact sheet for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement save live same renderer case sheet for TrainingService.
    # Called by: _validate
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement data worker init for TrainingService.
    # Called by: External callers and the owning workflow.
    # Calls: No same-class helper methods.
    def _data_worker_init(self, worker_id: int) -> None:
    ...
    # Purpose: Implement sampler epoch for phase for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    def _sampler_epoch_for_phase(self, phase: str, epoch: int, config: V9Config, dataset_length: int) -> int:
    ...
    # Purpose: Implement build loader for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    def _build_loader(
        self,
    ...
    # Purpose: Implement iter batches for TrainingService.
    # Called by: _validate, train_v9
    # Calls: _move_batch
    def _iter_batches(
        self,
        ...
        *,
        ...
        return iter(
            self._move_batch(raw_batch, device, channels_last=channels_last)
            for raw_batch in loader
        )

    # Purpose: Implement production component modules for TrainingService.
    # Called by: _run_final_qualification, train_v9
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement register component forward hooks for TrainingService.
    # Called by: _run_final_qualification, train_v9
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement snapshot trainable parameters for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement parameter belongs to for TrainingService.
    # Called by: _component_epoch_evidence
    # Calls: No same-class helper methods.
    def _parameter_belongs_to(self, path: str, parameter_name: str) -> bool:
        return parameter_name == path or parameter_name.startswith(path + ".")

    # Purpose: Implement component gradient norms for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement component epoch evidence for TrainingService.
    # Called by: train_v9
    # Calls: _parameter_belongs_to
    def _component_epoch_evidence(
        ...
        for label, (path, module) in components.items():
            ...
            for name, before in stage_start.items():
                if not self._parameter_belongs_to(path, name):
                    continue
    ...
    # Purpose: Implement aggregate component participation for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement run final qualification for TrainingService.
    # Called by: train_v9
    # Calls: _move_batch, _production_component_modules, _register_component_forward_hooks
    def _run_final_qualification(
        ...
        *,
        ...
        components = self._production_component_modules(model)
    ...
    # Purpose: Implement host memory info for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    def _host_memory_info(self) -> tuple[int, int] | None:
    ...
    # Purpose: Implement is cuda oom for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement resolve amp dtype for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    def _resolve_amp_dtype(self, config: V9Config, device: torch.device) -> torch.dtype:
        if device.type != "cuda":
    ...
    # Purpose: Implement configure cuda for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    def _configure_cuda(self, config: V9Config, device: torch.device) -> None:
        if device.type != "cuda":
            return
    ...
    # Purpose: Implement convert model channels last for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    def _convert_model_channels_last(self, model: torch.nn.Module) -> torch.nn.Module:
    ...
    # Purpose: Implement build optimizer for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement restore optimizer implementation for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement parameter memory format for TrainingService.
    # Called by: _align_optimizer_state_to_parameters
    # Calls: No same-class helper methods.
    def _parameter_memory_format(self, parameter: torch.Tensor) -> torch.memory_format:
    ...
    # Purpose: Implement align optimizer state to parameters for TrainingService.
    # Called by: train_v9
    # Calls: _parameter_memory_format
    def _align_optimizer_state_to_parameters(
        ...
        for group in optimizer.param_groups:
            ...
            for parameter in group["params"]:
                ...
                memory_format = self._parameter_memory_format(parameter)
    ...
    # Purpose: Implement is fused layout error for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    def _is_fused_layout_error(self, error: RuntimeError) -> bool:
    ...
    # Purpose: Implement compile training model for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement teacher seam authority for TrainingService.
    # Called by: _forward_for_phase
    # Calls: No same-class helper methods.
    def _teacher_seam_authority(self, batch: dict[str, torch.Tensor], config: V9Config) -> torch.Tensor:
    ...
    # Purpose: Implement teacher boundary gate for TrainingService.
    # Called by: _attach_same_renderer_candidate_outputs, _attach_sdf_teacher_outputs, _forward_for_phase
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement aligned teacher sdf for TrainingService.
    # Called by: _attach_sdf_teacher_outputs, _forward_for_phase
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement attach same renderer candidate outputs for TrainingService.
    # Called by: _forward_for_phase
    # Calls: _teacher_boundary_gate
    def _attach_same_renderer_candidate_outputs(
        ...
        """
        gate = self._teacher_boundary_gate(batch, config)
    ...
    # Purpose: Implement attach sdf teacher outputs for TrainingService.
    # Called by: _forward_for_phase
    # Calls: _aligned_teacher_sdf, _teacher_boundary_gate
    def _attach_sdf_teacher_outputs(
        ...
        with torch.no_grad():
            teacher = teacher_model._training_render_sdf_teacher(
                batch["input"], self._aligned_teacher_sdf(batch), gate, hardness
            )
    ...
    # Purpose: Implement parametric b1b train losses for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement forward for phase for TrainingService.
    # Called by: _validate, train_v9
    # Calls: _aligned_teacher_sdf, _attach_same_renderer_candidate_outputs, _attach_sdf_teacher_outputs, _teacher_boundary_gate, _teacher_seam_authority
    def _forward_for_phase(
        ...
        if phase in {"sdf-bootstrap", "sdf-proof"}:
            outputs = model(batch["input"])
            outputs = self._attach_same_renderer_candidate_outputs(outputs, model, batch, config)
            return self._attach_sdf_teacher_outputs(outputs, model, batch, config)

        if phase == "seam-proof":
            return model._forward_training(
                batch["input"],
                teacher_sdf=self._aligned_teacher_sdf(batch),
                teacher_gate=gate,
                teacher_hardness=hardness,
                teacher_seam_authority=self._teacher_seam_authority(batch, config),
        ...
        if phase == "gate-proof":
            ...
            outputs = self._attach_sdf_teacher_outputs(outputs, model, batch, config)
            return outputs
        return model(batch["input"])

    # Purpose: Implement validate for TrainingService.
    # Called by: train_v9
    # Calls: _abort_nonfinite, _finite_tensor, _forward_for_phase, _iter_batches, _oracle_render_match_numpy, _save_live_same_renderer_case_sheet, _status
    def _validate(
        self,
        model: FidelityResidualNetV9,
        loader: DataLoader,
        ...
        *,
        ...
        validation_batches = self._iter_batches(
            loader,
            device,
        ...
        with torch.no_grad():
            for batch_index, batch in enumerate(validation_batches, 1):
                ...
                if exact_geometry_metrics:
                    ...
                    oracle_match = self._oracle_render_match_numpy(
                    ...
                    if live_evidence_root is not None and phase in {"sdf-bootstrap", "sdf-proof"}:
                        self._save_live_same_renderer_case_sheet(
    ...
    # Purpose: Implement validate v992 architecture contract for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    def _validate_v992_architecture_contract(self, contract: Mapping[str, object]) -> None:
    ...
    # Purpose: Implement explicit primitive structure microproof for TrainingService.
    # Called by: _oracle_patch_structure_microproof, train_v9
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement oracle patch structure microproof for TrainingService.
    # Called by: External callers and the owning workflow.
    # Calls: _explicit_primitive_structure_microproof
    def _oracle_patch_structure_microproof(
        ...
        cfg.validate()
        return self._explicit_primitive_structure_microproof(device, cfg)

    # Purpose: Implement parametric structure microproof for TrainingService.
    # Called by: _direct_implicit_structure_microproof
    # Calls: No same-class helper methods.
    def _parametric_structure_microproof(self, device: torch.device) -> tuple[float, float, float, float]:
    ...
    # Purpose: Implement direct implicit structure microproof for TrainingService.
    # Called by: External callers and the owning workflow.
    # Calls: _parametric_structure_microproof
    def _direct_implicit_structure_microproof(self, device: torch.device) -> tuple[float, float]:
        before, after, _line, _curve = self._parametric_structure_microproof(device)
        return before, after

    # Purpose: Implement parametric junction microproof for TrainingService.
    # Called by: External callers and the owning workflow.
    # Calls: No same-class helper methods.
    def _parametric_junction_microproof(self, device: torch.device) -> tuple[float, float]:
    ...
    # Purpose: Implement phase seam sr microproof for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    def _phase_seam_sr_microproof(self, device: torch.device, config: V9Config) -> tuple[float, float, float]:
    ...
    # Purpose: Implement profile specialist microproof for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    def _profile_specialist_microproof(self, device: torch.device) -> tuple[float, float]:
    ...
    # Purpose: Implement structure selection rank for TrainingService.
    # Called by: train_v9
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement train v9 for TrainingService.
    # Called by: External callers and the owning workflow.
    # Calls: _abort_nonfinite, _aggregate_component_participation, _align_optimizer_state_to_parameters, _atomic_json, _atomic_torch_save, _bad_gradient_names, _bad_parameter_names, _build_loader, _build_optimizer, _capture_rng, _compile_training_model, _component_epoch_evidence, _component_gradient_norms, _config_hash, _configure_cuda, _convert_model_channels_last, _explicit_primitive_structure_microproof, _finite_tensor, _forward_for_phase, _host_memory_info, _is_cuda_oom, _is_fused_layout_error, _iter_batches, _parameters_are_finite, _parametric_b1b_substage, _parametric_b1b_train_losses, _phase_for_epoch, _phase_lr, _phase_seam_sr_microproof, _production_component_modules, _profile_specialist_microproof, _register_component_forward_hooks, _resolve_amp_dtype, _restore_optimizer_implementation, _restore_rng, _run_final_qualification, _sampler_epoch_for_phase, _save_contact_sheet, _snapshot_trainable_parameters, _status, _structure_selection_rank, _validate, _validate_v992_architecture_contract
    def train_v9(
        self,
        config: V9Config,
        repo_root: Path,
        ...
        *,
        ...
        config.validate()
        self._status("[startup] Trainer entered; configuration validated.")
        ...
        if restart:
            self._status(f"[startup] Restart requested; preparing prior-state backup in {output_dir}...")
            ...
            self._status(f"[startup] Restart confirmed; clearing active prior state in {output_dir}...")
        ...
        self._status("[startup] Resolving CUDA device and runtime settings...")
        device = resolve_device(config, requested_device)
        self._configure_cuda(config, device)
        ...
        self._status("[startup] Loading V9 dataset manifest...")
        manifest = load_dataset_manifest(repo_root, config)
        fingerprint = dataset_fingerprint(manifest, config)
        self._status("[startup] Dataset manifest loaded; constructing training/validation datasets...")
        ...
        seam_proof_train_loader = self._build_loader(
        ...
        synthetic_validation_loader = self._build_loader(
        ...
        self._status("[startup] Allocating V10.7.9 explicit-parametric deterministic-redraw model on training device...")
        model_started = time.perf_counter()
        model = FidelityResidualNetV9(config).to(device)
        contract = model.architecture_contract()
        self._validate_v992_architecture_contract(contract)
        structure_before, structure_after, line_jitter, curve_rough = self._explicit_primitive_structure_microproof(device, config)
        self._status(
        ...
        micro_before, micro_after = self._profile_specialist_microproof(device)
        self._status(
        ...
        if bool(getattr(config, "seam_microproof_enabled", True)) and bool(getattr(config, "raven_full_pipeline_preview_enabled", False)):
            seam_before, seam_after, seam_recovery = self._phase_seam_sr_microproof(device, config)
            self._status(
        ...
        if device.type == "cuda" and config.channels_last:
            model = self._convert_model_channels_last(model)
        self._status("[startup] Building optimizer and AMP state...")
        optimizer_started = time.perf_counter()
        optimizer, optimizer_mode = self._build_optimizer(model, config, device)
        amp_dtype = self._resolve_amp_dtype(config, device)
        use_amp = device.type == "cuda"
        ...
        if resume:
            ...
            if state.get("config_hash") != self._config_hash(config) or state.get("dataset_fingerprint") != fingerprint:
                raise RuntimeError("V9 resume state does not match the current semantic config or dataset")
            model.load_state_dict(state["state_dict"], strict=True)
            optimizer.load_state_dict(state["optimizer"])
            self._restore_optimizer_implementation(optimizer, optimizer_mode)
            optimizer_state_migrations = self._align_optimizer_state_to_parameters(optimizer)
        ...
        self._status("[startup] Preparing execution model...")
        execution_model, compile_status = self._compile_training_model(
            model, config.torch_compile_mode if device.type == "cuda" else "off"
        )
        self._status("[startup] Initialization complete; entering epoch loop.")
        ...
        self._status("=" * 68)
        self._status("NSAMDR V10.7.9 STAGED PARAMETRIC PRIMITIVE REDRAW")
        self._status("=" * 68)
        self._status(f"Device                   : {device}")
        self._status(f"Architecture schema      : {MODEL_SCHEMA}")
        self._status("Model                    : learned parametric geometry -> analytic SDF -> boundary/profile -> phase seam -> detail -> selector")
        self._status(f"Tile geometry            : {config.tile_size} LR -> {config.tile_size * 4} HR")
        self._status(f"Widths                   : {list(config.widths)}")
        self._status(f"Parameters               : {parameter_count(model):,}")
        self._status("Geometry outputs         : shared movable contour nodes + tangent lines -> cubic Hermite metric SDF + edge/orientation/hardness")
        self._status("Geometry RGB authority   : NONE")
        self._status(f"Detail reconstruction    : {bool(getattr(config, 'detail_reconstruction_enabled', True))} (explicit 2x/4x decoder)")
        self._status("Boundary primitive       : staged oracle/predicted SDF + two-sided sub-pixel coverage")
        self._status("Boundary sampling        : best-of-%d at %.0f%% probability" % (
            config.boundary_candidate_count, config.boundary_sampling_probability * 100.0))
        self._status(f"Exact geometry training   : {config.synthetic_geometry_probability * 100.0:.0f}% of training tiles")
        self._status("Contour objective         : B1a topology -> B1b LR-only inverse geometry fit -> B2 same-renderer redraw")
        if config.loss_precision == "mixed" and use_amp:
            self._status("Loss numerics            : reduced-precision kernels, FP32 scalar reductions")
        else:
            self._status("Loss numerics            : complete objective in FP32")
        self._status(f"Training state           : {state_path}")
        self._status(f"Performance profile      : {config.performance_profile}")
        self._status(f"AMP compute precision    : {precision_name}")
        self._status(f"Convolution layout       : {'channels-last' if config.channels_last and use_amp else 'contiguous'}")
        self._status(f"Optimizer implementation : {optimizer_mode}")
        self._status(f"Learning-rate scheduler  : {config.scheduler_name}")
        self._status(f"torch.compile            : {compile_status}")
        if resume:
            self._status(f"Optimizer state migration: {optimizer_state_migrations} moment tensor(s) aligned")
        self._status(f"Data pipeline            : workers={workers} prefetch={config.data_loader_prefetch_factor} persistent={workers > 0 and config.data_loader_persistent_workers}")
        self._status(
            "Training sample schedule : B1b cycles fixed analytic bank; other phases use deterministic epoch blocks"
        )
        self._status(f"CUDA batch prefetch      : {effective_cuda_prefetch}")
        if device.type == "cuda":
            ...
            if memory_governor.enabled:
                ...
                host_memory = self._host_memory_info()
                ...
                self._status(f"CUDA memory policy      : non-reactive ({total_vram:.1f} GiB device)")
        self._status(f"cuDNN benchmark / TF32   : {config.cudnn_benchmark} / {config.allow_tf32}")
        if resume:
            self._status(f"Resume epoch             : {start_epoch:03d}/{config.total_epochs:03d}")
        self._status("=" * 68)
        ...
        for epoch in range(start_epoch, config.total_epochs + 1):
            ...
            if planned_phase == "sdf-proof" and not topology_bootstrapped:
                self._status("  REJECTED_B1A_TOPOLOGY — B1b/B2 blocked because topology bootstrap was unsafe.")
                break
            ...
            if planned_phase == "seam-proof" and not render_qualified and not allow_preview_downstream:
                self._status("  REJECTED_B2_REDRAW — later stages blocked because same-renderer redraw did not qualify.")
                break
            if planned_phase == "seam-proof" and not render_qualified and allow_preview_downstream:
                self._status("  B2 not promotion-qualified; continuing Raven downstream training for diagnostic preview only.")
            ...
            if planned_phase == "seam-authority" and not seam_reconstruction_qualified and allow_preview_downstream:
                self._status("  B3 not promotion-qualified; continuing authority training for diagnostic preview.")
            if planned_phase == "gate-proof" and not seam_authority_qualified and not allow_preview_downstream:
                self._status("  REJECTED_B4_AUTHORITY — later stages blocked because seam detection failed.")
                break
            if planned_phase == "gate-proof" and not seam_authority_qualified and allow_preview_downstream:
                self._status("  B4 not promotion-qualified; continuing profile/detail training for diagnostic preview.")
            ...
            if planned_phase == "physical-finetune" and phase == "detail-reconstruction" and previous_phase != "detail-reconstruction":
                self._status("  Detail proof not yet qualified; reallocating selector epochs to full-resolution detail reconstruction.")
            ...
            if participation_stage_phase != phase:
                ...
                participation_stage_start = self._snapshot_trainable_parameters(model)
            component_modules = self._production_component_modules(model)
            component_forward_counts, component_hook_handles = (
                self._register_component_forward_hooks(component_modules)
            )
            ...
            model.train()
            learning_rate = self._phase_lr(phase, config, epoch)
            seam_proof_passes = 1
            if phase == "seam-proof":
                ...
                for pass_index in range(seam_proof_passes):
                    if isinstance(epoch_loader.sampler, _EpochOffsetSampler):
                        epoch_loader.sampler.set_epoch(
                            self._sampler_epoch_for_phase(phase, epoch, config, len(epoch_loader.dataset))
                            + pass_index
                        )
            ...
            for batch_index, batch in enumerate(train_batches, 1):
                ...
                while not step_completed:
                    ...
                    try:
                        for retry in range(config.amp_overflow_retries + 1):
                            ...
                            batch_component_gradients = self._component_gradient_norms(
                                component_modules
                            )
                            ...
                            if not gradient_finite:
                                ...
                                bad_gradient_names = (
                                    self._bad_gradient_names(model)
                            ...
                            try:
                                ...
                                if (
                                    ...
                                    or not self._is_fused_layout_error(error)
                                ):
                                    raise
                                migrated = self._align_optimizer_state_to_parameters(
                                    optimizer
                                )
                                if migrated <= 0:
                                    raise
                        ...
                        if (
                            ...
                            or not self._is_cuda_oom(error)
                ...
                if global_step % config.parameter_finite_check_interval == 0:
                    if not self._parameters_are_finite(model):
                        self._abort_nonfinite(
            ...
            validation_metrics, validation_outputs, validation_batch = self._validate(
            ...
            if (
                ...
                synthetic_sdf_metrics, _synthetic_outputs, _synthetic_batch = self._validate(
            ...
            self._save_contact_sheet(validation_outputs, validation_batch, sample_path, phase)

            for hook_handle in component_hook_handles:
                hook_handle.remove()
            epoch_component_evidence = self._component_epoch_evidence(
                model,
                component_modules,
            ...
            if synthetic_sdf_metrics is not None:
                ...
                if phase == "sdf-bootstrap":
                    ...
                    if select_structure:
                        ...
                        self._atomic_torch_save(payload, best_structure_path)
                        self._atomic_torch_save(payload, best_b1b_path)
                        self._status(
                ...
                if phase == "seam-proof":
                    ...
                    if seam_recovery >= float(getattr(config, "seam_forced_recovery_required", 0.70)) and not seam_reconstruction_qualified:
                        ...
                        self._status(f"  B3 FORCED-AUTHORITY SEAM PROOF PASSED: recovery={seam_recovery:+.1%}.")
                if phase == "seam-authority":
                    ...
                    if seam_iou >= float(getattr(config, "seam_authority_iou_required", 0.55)) and not seam_authority_qualified:
                        ...
                        self._status(f"  B4 SEAM AUTHORITY PROOF PASSED: IoU={seam_iou:.3f}.")
                if phase == "seam-proof":
        ...
        if stop_after_phase in isolated_stop_phases or not best_path.is_file():
            ...
            if selected_stage is not None:
                ...
                self._atomic_torch_save(wrapper, best_path)
                self._status(
        ...
        final_qualification = self._run_final_qualification(
            model,
            validation_loader,
            config,
            device,
        ...
        architecture_participation_summary = self._aggregate_component_participation(
        ...
        if stop_after_phase in isolated_stop_phases and selected_is_isolated and state_path.is_file():
            ...
            self._atomic_torch_save(resume_state, state_path)
            self._status(
        ...
        self._atomic_json(metadata, metadata_path)
        print("=" * 68)
...
_status = _training_service._status
_render_profile_metrics_numpy = _training_service._render_profile_metrics_numpy
_oracle_render_match_numpy = _training_service._oracle_render_match_numpy
_config_hash = _training_service._config_hash
_phase_for_epoch = _training_service._phase_for_epoch
_parametric_b1b_substage = _training_service._parametric_b1b_substage
_phase_lr = _training_service._phase_lr
_atomic_torch_save = _training_service._atomic_torch_save
_atomic_json = _training_service._atomic_json
_move_batch = _training_service._move_batch
_finite_tensor = _training_service._finite_tensor
_bad_gradient_names = _training_service._bad_gradient_names
_bad_parameter_names = _training_service._bad_parameter_names
_parameters_are_finite = _training_service._parameters_are_finite
_abort_nonfinite = _training_service._abort_nonfinite
_capture_rng = _training_service._capture_rng
_rng_byte_tensor = _training_service._rng_byte_tensor
_restore_rng = _training_service._restore_rng
_save_contact_sheet = _training_service._save_contact_sheet
_save_live_same_renderer_case_sheet = _training_service._save_live_same_renderer_case_sheet
_data_worker_init = _training_service._data_worker_init
_sampler_epoch_for_phase = _training_service._sampler_epoch_for_phase
_build_loader = _training_service._build_loader
_iter_batches = _training_service._iter_batches
_production_component_modules = _training_service._production_component_modules
_register_component_forward_hooks = _training_service._register_component_forward_hooks
_snapshot_trainable_parameters = _training_service._snapshot_trainable_parameters
_parameter_belongs_to = _training_service._parameter_belongs_to
_component_gradient_norms = _training_service._component_gradient_norms
_component_epoch_evidence = _training_service._component_epoch_evidence
_aggregate_component_participation = _training_service._aggregate_component_participation
_run_final_qualification = _training_service._run_final_qualification
_host_memory_info = _training_service._host_memory_info
_is_cuda_oom = _training_service._is_cuda_oom
_resolve_amp_dtype = _training_service._resolve_amp_dtype
_configure_cuda = _training_service._configure_cuda
_convert_model_channels_last = _training_service._convert_model_channels_last
_build_optimizer = _training_service._build_optimizer
_restore_optimizer_implementation = _training_service._restore_optimizer_implementation
_parameter_memory_format = _training_service._parameter_memory_format
_align_optimizer_state_to_parameters = _training_service._align_optimizer_state_to_parameters
_is_fused_layout_error = _training_service._is_fused_layout_error
_compile_training_model = _training_service._compile_training_model
_teacher_seam_authority = _training_service._teacher_seam_authority
_teacher_boundary_gate = _training_service._teacher_boundary_gate
_aligned_teacher_sdf = _training_service._aligned_teacher_sdf
_attach_same_renderer_candidate_outputs = _training_service._attach_same_renderer_candidate_outputs
_attach_sdf_teacher_outputs = _training_service._attach_sdf_teacher_outputs
_parametric_b1b_train_losses = _training_service._parametric_b1b_train_losses
_forward_for_phase = _training_service._forward_for_phase
_validate = _training_service._validate
_validate_v992_architecture_contract = _training_service._validate_v992_architecture_contract
_explicit_primitive_structure_microproof = _training_service._explicit_primitive_structure_microproof
_oracle_patch_structure_microproof = _training_service._oracle_patch_structure_microproof
_parametric_structure_microproof = _training_service._parametric_structure_microproof
_direct_implicit_structure_microproof = _training_service._direct_implicit_structure_microproof
_parametric_junction_microproof = _training_service._parametric_junction_microproof
_phase_seam_sr_microproof = _training_service._phase_seam_sr_microproof
_profile_specialist_microproof = _training_service._profile_specialist_microproof
_structure_selection_rank = _training_service._structure_selection_rank
train_v9 = _training_service.train_v9
...
class _MetricAccumulator:
    ...
    # Purpose: Implement init for _MetricAccumulator.
    # Called by: External callers and the owning workflow.
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement add for _MetricAccumulator.
    # Called by: External callers and the owning workflow.
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement average value for _MetricAccumulator.
    # Called by: External callers and the owning workflow.
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement averages for _MetricAccumulator.
    # Called by: External callers and the owning workflow.
    # Calls: No same-class helper methods.
...
class _EpochOffsetSampler(Sampler[int]):
    ...
    # Purpose: Implement init for _EpochOffsetSampler.
    # Called by: External callers and the owning workflow.
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement set epoch for _EpochOffsetSampler.
    # Called by: External callers and the owning workflow.
    # Calls: No same-class helper methods.
    def set_epoch(self, epoch: int) -> None:
        self.epoch = max(0, int(epoch))

    # Purpose: Implement iter for _EpochOffsetSampler.
    # Called by: External callers and the owning workflow.
    # Calls: No same-class helper methods.
    def __iter__(self):
    ...
    # Purpose: Implement len for _EpochOffsetSampler.
    # Called by: External callers and the owning workflow.
    # Calls: No same-class helper methods.
...
class _CudaPrefetcher(Iterator[dict[str, torch.Tensor]]):
    ...
    # Purpose: Implement preload for _CudaPrefetcher.
    # Called by: __init__, __next__
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement init for _CudaPrefetcher.
    # Called by: External callers and the owning workflow.
    # Calls: _preload
    def __init__(
        self,
    ...
    # Purpose: Implement iter for _CudaPrefetcher.
    # Called by: External callers and the owning workflow.
    # Calls: No same-class helper methods.
    def __iter__(self) -> "_CudaPrefetcher":
        return self

    # Purpose: Implement next for _CudaPrefetcher.
    # Called by: External callers and the owning workflow.
    # Calls: _preload
...
class _ReactiveCudaMemoryGovernor:
    ...
    # Purpose: Implement init for _ReactiveCudaMemoryGovernor.
    # Called by: External callers and the owning workflow.
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement gib for _ReactiveCudaMemoryGovernor.
    # Called by: _release_cache, _resource_text, _set_mode, _wait_for_recovery, before_step, handle_oom, status_text
    # Calls: No same-class helper methods.
    @staticmethod
    def _gib(value: int | float) -> float:
        return float(value) / (1024.0 ** 3)

    # Purpose: Implement free bytes for _ReactiveCudaMemoryGovernor.
    # Called by: _release_cache, _stable_resource_snapshot, _wait_for_recovery, after_step, handle_oom, status_text
    # Calls: No same-class helper methods.
    def _free_bytes(self) -> int:
        free_bytes, _ = torch.cuda.mem_get_info(self.device)
        return int(free_bytes)

    # Purpose: Implement our reserved bytes for _ReactiveCudaMemoryGovernor.
    # Called by: _physical_live_budget
    # Calls: No same-class helper methods.
    def _our_reserved_bytes(self) -> int:
        return int(torch.cuda.memory_reserved(self.device))

    # Purpose: Implement our allocated bytes for _ReactiveCudaMemoryGovernor.
    # Called by: _gpu_mode_feasible, before_step
    # Calls: No same-class helper methods.
    def _our_allocated_bytes(self) -> int:
        return int(torch.cuda.memory_allocated(self.device))

    # Purpose: Implement required free bytes for _ReactiveCudaMemoryGovernor.
    # Called by: before_step, handle_oom
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement physical live budget for _ReactiveCudaMemoryGovernor.
    # Called by: before_step
    # Calls: _our_reserved_bytes
    ...
    # Purpose: Implement gpu mode feasible for _ReactiveCudaMemoryGovernor.
    # Called by: before_step
    # Calls: _our_allocated_bytes
    ...
    # Purpose: Implement release cache for _ReactiveCudaMemoryGovernor.
    # Called by: after_step, before_step
    # Calls: _free_bytes, _gib
    ...
    # Purpose: Implement set mode for _ReactiveCudaMemoryGovernor.
    # Called by: _wait_for_recovery, after_step, before_step, handle_oom
    # Calls: _gib
    ...
    # Purpose: Implement host free bytes for _ReactiveCudaMemoryGovernor.
    # Called by: _host_offload_safe, _resource_text, before_step
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement host offload safe for _ReactiveCudaMemoryGovernor.
    # Called by: _wait_for_recovery, before_step, handle_oom
    # Calls: _host_free_bytes
    ...
    # Purpose: Implement resource text for _ReactiveCudaMemoryGovernor.
    # Called by: _wait_for_recovery
    # Calls: _gib, _host_free_bytes
    ...
    # Purpose: Implement wait for recovery for _ReactiveCudaMemoryGovernor.
    # Called by: External callers and the owning workflow.
    # Calls: _free_bytes, _gib, _host_offload_safe, _resource_text, _set_mode
    ...
    # Purpose: Implement before step for _ReactiveCudaMemoryGovernor.
    # Called by: External callers and the owning workflow.
    # Calls: _gib, _gpu_mode_feasible, _host_free_bytes, _host_offload_safe, _our_allocated_bytes, _physical_live_budget, _release_cache, _required_free_bytes, _set_mode, _stable_resource_snapshot
    ...
    # Purpose: Implement autograd context for _ReactiveCudaMemoryGovernor.
    # Called by: External callers and the owning workflow.
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement after step for _ReactiveCudaMemoryGovernor.
    # Called by: External callers and the owning workflow.
    # Calls: _free_bytes, _release_cache, _set_mode
    ...
    # Purpose: Implement handle oom for _ReactiveCudaMemoryGovernor.
    # Called by: External callers and the owning workflow.
    # Calls: _free_bytes, _gib, _host_offload_safe, _required_free_bytes, _set_mode
    ...
    # Purpose: Implement status text for _ReactiveCudaMemoryGovernor.
    # Called by: External callers and the owning workflow.
    # Calls: _free_bytes, _gib
