...
class AuthoredTextureDatasetConfig(Protocol):
    ...
    # Purpose: Implement validate for AuthoredTextureDatasetConfig.
...
class AuthoredTextureDatasetRepository:
    # Purpose: Implement stable fraction for AuthoredTextureDatasetRepository.
    # Called by: _family_split
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement family split for AuthoredTextureDatasetRepository.
    # Called by: discover_extracted_families, discover_shared_cache_families
    # Calls: _stable_fraction
    def _family_split(self, stem: str, validation_fraction: float) -> str:
        return "validation" if self._stable_fraction(stem.lower()) < validation_fraction else "train"

    # Purpose: Implement strip role suffix for AuthoredTextureDatasetRepository.
    # Called by: discover_extracted_families, discover_shared_cache_families
    # Calls: No same-class helper methods.
    def _strip_role_suffix(self, logical: str) -> tuple[str, str] | None:
    ...
    # Purpose: Implement parse dds header for AuthoredTextureDatasetRepository.
    # Called by: discover_extracted_families, discover_shared_cache_families
    # Calls: No same-class helper methods.
    def parse_dds_header(self, path: Path) -> tuple[int, int, int, str]:
    ...
    # Purpose: Implement cache fingerprint for AuthoredTextureDatasetRepository.
    # Called by: discover_shared_cache_families
    # Calls: No same-class helper methods.
    def _cache_fingerprint(self, indexes: Sequence[Path], config: AuthoredTextureDatasetConfig, source_root: str) -> str:
    ...
    # Purpose: Implement import eve asset helpers for AuthoredTextureDatasetRepository.
    # Called by: _decode_source, discover_shared_cache_families, prepare_dataset
    # Calls: No same-class helper methods.
    def _import_eve_asset_helpers(self, repo_root: Path):
        if str(repo_root) not in sys.path:
            sys.path.insert(0, str(repo_root))
        from tools.nsamdr import eve_asset_test  # type: ignore
        return eve_asset_test

    # Purpose: Implement role minimum for AuthoredTextureDatasetRepository.
    # Called by: discover_extracted_families, discover_shared_cache_families
    # Calls: No same-class helper methods.
    def _role_minimum(self, config: AuthoredTextureDatasetConfig, role: str) -> int:
        return config.min_source_dimension if role == "albedo" else config.min_auxiliary_dimension

    # Purpose: Implement prefer larger for AuthoredTextureDatasetRepository.
    # Called by: discover_extracted_families, discover_shared_cache_families
    # Calls: No same-class helper methods.
    def _prefer_larger(self, previous: TextureSource | None, candidate: TextureSource) -> TextureSource:
        if previous is None:
            return candidate
    ...
    # Purpose: Implement map suffix histogram for AuthoredTextureDatasetRepository.
    # Called by: discover_shared_cache_families
    # Calls: No same-class helper methods.
    def _map_suffix_histogram(self, rows: Sequence[object]) -> dict[str, int]:
    ...
    # Purpose: Implement dimension histogram for AuthoredTextureDatasetRepository.
    # Called by: discover_shared_cache_families
    # Calls: No same-class helper methods.
    def _dimension_histogram(self, values: Sequence[TextureSource]) -> dict[str, int]:
        counts: Counter[str] = Counter(f"{value.width}x{value.height}" for value in values)
        return dict(sorted(counts.items(), key=lambda item: (-item[1], item[0])))

    # Purpose: Implement family missing roles for AuthoredTextureDatasetRepository.
    # Called by: discover_shared_cache_families
    # Calls: No same-class helper methods.
    def _family_missing_roles(self, grouped: dict[str, dict[str, TextureSource]], limit: int = 24) -> list[dict[str, object]]:
    ...
    # Purpose: Implement discover shared cache families for AuthoredTextureDatasetRepository.
    # Called by: prepare_dataset
    # Calls: _cache_fingerprint, _dimension_histogram, _family_missing_roles, _family_split, _import_eve_asset_helpers, _map_suffix_histogram, _prefer_larger, _role_minimum, _strip_role_suffix, parse_dds_header
    def discover_shared_cache_families(
        ...
        helpers = self._import_eve_asset_helpers(repo_root)
        ...
        for row in rows:
            stats["indexRows"] += 1
            parsed = self._strip_role_suffix(row.logical)
            if parsed is None:
                continue
            stem, role = parsed
            ...
            try:
                width, height, mip_count, format_name = self.parse_dds_header(source_path)
            ...
            all_bucket[role] = self._prefer_larger(all_bucket.get(role), candidate)

            minimum = self._role_minimum(config, role)
            ...
            accepted_bucket[role] = self._prefer_larger(accepted_bucket.get(role), candidate)
        ...
        for stem, roles in grouped_accepted.items():
            ...
            families.append(PBRFamily(
                family_id=family_id,
                stem=stem,
                split=self._family_split(stem, config.validation_fraction),
                albedo=roles["albedo"],
                normal=roles["normal"],
        ...
        audit = {
            ...
            "roleDimensionsAll": {role: self._dimension_histogram(values) for role, values in role_sources_all.items()},
            "roleDimensionsAccepted": {role: self._dimension_histogram(values) for role, values in role_sources_accepted.items()},
            "mapSuffixCounts": self._map_suffix_histogram(rows),
            ...
            "missingRoleExamples": self._family_missing_roles(grouped_accepted),
        ...
        metadata = {
            ...
            "fingerprint": self._cache_fingerprint(indexes, config, str(cache_root)),
        }
        return selected, metadata

    # Purpose: Implement discover extracted families for AuthoredTextureDatasetRepository.
    # Called by: prepare_dataset
    # Calls: _family_split, _prefer_larger, _role_minimum, _strip_role_suffix, parse_dds_header
    def discover_extracted_families(self, source_root: Path, config: AuthoredTextureDatasetConfig) -> tuple[list[PBRFamily], dict[str, object]]:
        ...
        for path in files:
            parsed = self._strip_role_suffix(path.name.lower())
            if parsed is None:
                continue
            ...
            try:
                if path.suffix.lower() == ".dds":
                    width, height, mip_count, format_name = self.parse_dds_header(path)
                else:
            ...
            bucket[role] = self._prefer_larger(bucket.get(role), candidate)
        ...
        for stem, roles in grouped.items():
            ...
            family_id = hashlib.sha1(stem.encode("utf-8")).hexdigest()[:16]
            families.append(PBRFamily(
                family_id, stem, self._family_split(stem, config.validation_fraction),
    ...
    # Purpose: Implement decode source for AuthoredTextureDatasetRepository.
    # Called by: prepare_dataset
    # Calls: _import_eve_asset_helpers
    def _decode_source(
        self,
    ...
    # Purpose: Implement load rgb for AuthoredTextureDatasetRepository.
    # Called by: prepare_dataset
    # Calls: No same-class helper methods.
    ...
    # Purpose: Implement renormalize normal xy for AuthoredTextureDatasetRepository.
    # Called by: load_normal_training_rgb
    # Calls: No same-class helper methods.
    def _renormalize_normal_xy(self, normal_xy: np.ndarray) -> np.ndarray:
        length = np.sqrt(np.maximum((normal_xy ** 2).sum(axis=-1, keepdims=True), 1.0e-8))
        return normal_xy / np.maximum(1.0, length / 0.999)

    # Purpose: Implement load normal training rgb for AuthoredTextureDatasetRepository.
    # Called by: prepare_dataset
    # Calls: _renormalize_normal_xy
    def load_normal_training_rgb(self, path: Path) -> tuple[np.ndarray, str]:
        ...
        xy = self._renormalize_normal_xy(xy)
    ...
    # Purpose: Implement resize rgb for AuthoredTextureDatasetRepository.
    # Called by: _crop_aligned
    # Calls: No same-class helper methods.
    def _resize_rgb(self, array: np.ndarray, size: tuple[int, int], *, nearest: bool = False) -> np.ndarray:
    ...
    # Purpose: Implement crop aligned for AuthoredTextureDatasetRepository.
    # Called by: prepare_dataset
    # Calls: _resize_rgb
    def _crop_aligned(self, array: np.ndarray, normalized_box: tuple[float, float, float, float], output_size: int, *, nearest: bool) -> np.ndarray:
        ...
        return self._resize_rgb(cropped, (output_size, output_size), nearest=nearest)

    # Purpose: Implement detail map for AuthoredTextureDatasetRepository.
    # Called by: _select_crop_boxes
    # Calls: No same-class helper methods.
    def detail_map(self, albedo: np.ndarray, normal: np.ndarray, material: np.ndarray) -> np.ndarray:
    ...
    # Purpose: Implement score box for AuthoredTextureDatasetRepository.
    # Called by: _select_crop_boxes
    # Calls: No same-class helper methods.
    def _score_box(self, score: np.ndarray, box: tuple[float, float, float, float]) -> float:
    ...
    # Purpose: Implement select crop boxes for AuthoredTextureDatasetRepository.
    # Called by: prepare_dataset
    # Calls: _score_box, detail_map
    def _select_crop_boxes(
        ...
        score = self.detail_map(albedo, normal, material)
        ...
        for _ in range(max(96, count * 16)):
            ...
            candidates.append((self._score_box(score, box), box))
    ...
    # Purpose: Implement write discovery report for AuthoredTextureDatasetRepository.
    # Called by: prepare_dataset
    # Calls: No same-class helper methods.
    def _write_discovery_report(self, output_root: Path, source_metadata: dict[str, object]) -> Path:
        output_root.mkdir(parents=True, exist_ok=True)
        path = output_root / "discovery_report.json"
        path.write_text(json.dumps(source_metadata, indent=2) + "\n", encoding="utf-8")
        return path

    # Purpose: Implement print discovery audit for AuthoredTextureDatasetRepository.
    # Called by: prepare_dataset
    # Calls: No same-class helper methods.
    def _print_discovery_audit(self, source_metadata: dict[str, object], report_path: Path) -> None:
    ...
    # Purpose: Implement prepare dataset for AuthoredTextureDatasetRepository.
    # Called by: External callers and the owning workflow.
    # Calls: _crop_aligned, _decode_source, _import_eve_asset_helpers, _load_rgb, _print_discovery_audit, _select_crop_boxes, _write_discovery_report, discover_extracted_families, discover_shared_cache_families, load_normal_training_rgb
    def prepare_dataset(
        self,
        repo_root: Path,
        config: AuthoredTextureDatasetConfig,
        *,
        ...
        if source_root is not None:
            families, source_metadata = self.discover_extracted_families(source_root.resolve(), config)
        else:
            families, source_metadata = self.discover_shared_cache_families(repo_root, config, shared_cache)
        report_path = self._write_discovery_report(output_root, source_metadata)
        self._print_discovery_audit(source_metadata, report_path)
        if audit_only:
            return {
        ...
        with tempfile.TemporaryDirectory(prefix="nsamdr_authored_decode_") as temp_name:
            ...
            for family_index, family in enumerate(families, 1):
                ...
                for source in (family.albedo, family.normal, family.material):
                    if source is None:
                        continue
                    decoded[source.role] = self._decode_source(
                        source, temp / f"{family.family_id}_{source.role}.png", repo_root, converter)
                albedo = self._load_rgb(decoded["albedo"])
                normal, normal_encoding = self.load_normal_training_rgb(decoded["normal"])
                material_valid = family.material is not None and "material" in decoded
                if material_valid:
                    material = self._load_rgb(decoded["material"])
                else:
                ...
                for crop_index, (box, detail_score) in enumerate(boxes):
                    ...
                    albedo_crop = self._crop_aligned(albedo, box, config.source_crop_size, nearest=False)
                    normal_crop = self._crop_aligned(normal, box, config.source_crop_size, nearest=False)
                    material_crop = self._crop_aligned(material, box, config.source_crop_size, nearest=True)
                    metadata = {
                        "schema": CROP_SCHEMA,
                        "cropId": crop_id,
...
_authored_texture_dataset_repository = AuthoredTextureDatasetRepository()
_stable_fraction = _authored_texture_dataset_repository._stable_fraction
_family_split = _authored_texture_dataset_repository._family_split
_strip_role_suffix = _authored_texture_dataset_repository._strip_role_suffix
parse_dds_header = _authored_texture_dataset_repository.parse_dds_header
_cache_fingerprint = _authored_texture_dataset_repository._cache_fingerprint
_import_eve_asset_helpers = _authored_texture_dataset_repository._import_eve_asset_helpers
_role_minimum = _authored_texture_dataset_repository._role_minimum
_prefer_larger = _authored_texture_dataset_repository._prefer_larger
_map_suffix_histogram = _authored_texture_dataset_repository._map_suffix_histogram
_dimension_histogram = _authored_texture_dataset_repository._dimension_histogram
_family_missing_roles = _authored_texture_dataset_repository._family_missing_roles
discover_shared_cache_families = _authored_texture_dataset_repository.discover_shared_cache_families
discover_extracted_families = _authored_texture_dataset_repository.discover_extracted_families
_decode_source = _authored_texture_dataset_repository._decode_source
_load_rgb = _authored_texture_dataset_repository._load_rgb
_renormalize_normal_xy = _authored_texture_dataset_repository._renormalize_normal_xy
load_normal_training_rgb = _authored_texture_dataset_repository.load_normal_training_rgb
_resize_rgb = _authored_texture_dataset_repository._resize_rgb
_crop_aligned = _authored_texture_dataset_repository._crop_aligned
detail_map = _authored_texture_dataset_repository.detail_map
_score_box = _authored_texture_dataset_repository._score_box
_select_crop_boxes = _authored_texture_dataset_repository._select_crop_boxes
_write_discovery_report = _authored_texture_dataset_repository._write_discovery_report
_print_discovery_audit = _authored_texture_dataset_repository._print_discovery_audit
prepare_dataset = _authored_texture_dataset_repository.prepare_dataset
