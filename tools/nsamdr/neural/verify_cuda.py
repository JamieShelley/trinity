...
class VerifyCudaService:
    # Purpose: Implement status for VerifyCudaService.
    # Called by: main
    # Calls: No same-class helper methods.
    def _status(self, message: str) -> None:
        print(message, flush=True)

    # Purpose: Implement main for VerifyCudaService.
    ...
    def main(self) -> int:
        ...
        self._status("[cuda-preflight] Python process entered.")
        self._status("[cuda-preflight] Importing PyTorch/CUDA runtime DLLs...")
        ...
        self._status(f"PyTorch version: {torch.__version__}")
        self._status(f"PyTorch CUDA runtime: {torch.version.cuda or 'none'}")

        self._status("[cuda-preflight] Querying CUDA driver...")
        ...
        self._status(f"CUDA available: {cuda_available}")
        ...
        self._status(f"[cuda-preflight] Initializing CUDA context on {device}...")
        ...
        self._status(f"CUDA device: {device} - {props.name}")
        self._status(f"Compute capability: {props.major}.{props.minor} ({capability})")
        self._status(f"VRAM: {props.total_memory / (1024 ** 3):.1f} GiB")
        self._status(
        ...
        self._status("[cuda-preflight] Checking compiled CUDA architectures...")
        ...
        self._status(f"Compiled architectures: {' '.join(architectures)}")
        ...
        self._status(f"CUDA matrix test passed; checksum={checksum:.6f}")
        self._status("[cuda-preflight] Preflight complete.")
        return 0

_verify_cuda_service = VerifyCudaService()
_status = _verify_cuda_service._status
main = _verify_cuda_service.main
