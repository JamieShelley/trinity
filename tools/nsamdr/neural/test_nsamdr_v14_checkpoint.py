...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate one NSAMDR V15.0 checkpoint")
    ...
    if not checkpoint.is_file():
        raise SystemExit(f"missing V15.0 checkpoint: {checkpoint}")
    ...
    if payload.get("schema") != MODEL_SCHEMA:
        raise SystemExit("V15.0 checkpoint schema mismatch")
    ...
    for key in (
        ...
        if tuple(output[key].shape[-2:]) != (expected, expected):
            raise SystemExit(
                f"V15.0 checkpoint output shape mismatch for {key}: {tuple(output[key].shape)}"
            )
        if not bool(torch.isfinite(output[key]).all().item()):
            raise SystemExit(f"V15.0 checkpoint produced non-finite output: {key}")

    contract = model.architecture_contract()
    if (
        contract.get("schema") != MODEL_SCHEMA
        or contract.get("revision") != "V15.0"
        or contract.get("backbone") != "EDSR-style-single-resolution-HR"
        ...
        raise SystemExit("V15.0 architecture contract mismatch")

    if any(isinstance(module, torch.nn.PixelShuffle) for module in model.modules()):
        raise SystemExit("V15.0 checkpoint unexpectedly contains PixelShuffle")
    if any(isinstance(module, torch.nn.ConvTranspose2d) for module in model.modules()):
        raise SystemExit("V15.0 checkpoint unexpectedly contains ConvTranspose2d")
    if any(isinstance(module, torch.nn.BatchNorm2d) for module in model.modules()):
        raise SystemExit("V15.0 checkpoint unexpectedly contains BatchNorm2d")

    print("NSAMDR V15.0 checkpoint validation passed")
